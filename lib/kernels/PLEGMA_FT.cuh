...
// template<typename Float, int Flength>
// __global__ void fourier_transform_3D_kernel(Float* block, Float* in,int it, int sign){
//   int vid = blockIdx.x*blockDim.x + threadIdx.x;
//   int sid = vid + it*DGC_localVolume3D;
//   Float2<Float> *block2 = (Float2<Float> *)block;
//   if(vid >= DGC_localVolume3D) return;
//   generic2<Float> R(in);
//   Float2<Float> inBuff[Flength];
// #pragma unroll
//   for(int i = 0; i < Flength; i++)
//     inBuff[i] = R.get(i,sid);
//   extern __shared__ int ext_shared_cache[];
//   Float2<Float> *shared_cache = (Float2<Float> *) ext_shared_cache;
//   int source_pos[3] = {0,0,0};
//   fourier_transform_3D(block2,inBuff,shared_cache,Flength,vid,source_pos,0,sign);      
// }

// template<typename Float>
// static void fourier_transform_3D_k(PLEGMA_FT<Float> &ft,const PLEGMA_Field<Float> &field, int it, int sign){
//   void (* kernelMap)(Float*,Float*,int,int);
//   switch(field.Field_length()){
//   case 1:  kernelMap = fourier_transform_3D_kernel<Float,1>; break;
//   case N_COLS * N_COLS:  kernelMap = fourier_transform_3D_kernel<Float,N_COLS * N_COLS>; break;
//   case N_DIMS * N_COLS * N_COLS: kernelMap = fourier_transform_3D_kernel<Float,N_DIMS * N_COLS * N_COLS>; break;
//   case N_SPINS * N_COLS: kernelMap = fourier_transform_3D_kernel<Float,N_SPINS * N_COLS>; break;
//   case N_SPINS * N_COLS * N_SPINS * N_COLS: kernelMap = fourier_transform_3D_kernel<Float,N_SPINS * N_COLS * N_SPINS * N_COLS>; break;
//   case N_SPINS * N_SPINS: kernelMap = fourier_transform_3D_kernel<Float,N_SPINS * N_SPINS>; break;
//   default: PLEGMA_error("Template not instantiated");
//   }
...
//   tune(ps, "fourier_transform_3D_kernel", kernelMap, d_partial_block, field.D_elem(), it, sign);
//   size_t alloc_size=ft.Nmoms()*site_size*ps.tp.grid.x*2*sizeof(Float);
//   PLEGMA_printf("%d\n",ps.tp.grid.x);
//   cudaMalloc((void**)&d_partial_block, alloc_size);
//   checkCudaError();
//   run(ps, "fourier_transform_3D_kernel", kernelMap, d_partial_block, field.D_elem(), it, sign);
//   PLEGMA_printf("%d\n",ps.tp.grid.x);
...
static void fourier_transform_3D_k(PLEGMA_FT<Float> &ft,const PLEGMA_Field<Float> &field, int it, int sign){
  ...
  size_t alloc_size=ft.Nmoms()*site_size*gridDim.x*2*sizeof(Float);

  cudaMalloc((void**)&d_partial_block, alloc_size);
  checkCudaError();
  fourier_transform_3D_kernel<Float><<<gridDim,blockDim,THREADS_PER_BLOCK*2*sizeof(Float)>>>(d_partial_block, field.D_elem(),field.Field_length(), it, sign);
