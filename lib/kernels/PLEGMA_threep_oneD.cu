...
__global__ void threep_oneD_device(Float2<FloatC>* block2,
				   ...
				   int it, int time_step, int maxT, int4 source,
				    int signProps, bool runFT, tex_mom_list moms, int dir){
  ...
  if (sid3D < DGC_localVolume3D){
    ...
    partial_trace_mul_Prop_G_Prop<true,ZERO_PLUS,false>(R,prop1,prop2,su3);

    // - term x, x-dir^, x-dir
    /*prop1Tex.get(prop1,vid);*/ gaugeTex.get<Minus>(su3,dir,vid,dir); prop2Tex.get<Minus>(prop2,vid,dir);
    partial_trace_mul_Prop_G_Prop<true,ACC_MINUS,true>(R,prop1,prop2,su3);

    // + term x-dir, x-dir, x
    prop1Tex.get<Minus>(prop1,vid,dir); /*gaugeTex.get<Minus>(su3,dir,vid,dir);*/ prop2Tex.get(prop2,vid);
    ...
    for(int iop = 0; iop < listGammas.size; iop++){
      ...
      accum[iop] = 0.25*((signProps > 0) ? trace_gamma_S<true>(opId,TMP,R) : trace_gamma_S<true>(opId,TMM,R));
    }
  }
...
template<typename FloatC, typename FloatA, typename FloatB, typename FloatG>
__global__ void threep_oneD_zfac_device(Float2<FloatC>* block2,
					propTex<FloatA> prop1Tex, propTex<FloatB> prop2Tex,
					gaugeTex<FloatG> gaugeTex, KernelArr<GAMMAS> listGammas,
					int it, int time_step, int maxT, int4 source,
					int signProps, bool runFT, tex_mom_list moms,
					int mu, int nu, int c1, int c2, int dir){
  int grid3D = gridDim.x/time_step;
  int sid3D = (blockIdx.x % grid3D)*blockDim.x + threadIdx.x;
  int tid = blockIdx.x/grid3D;
  // this takes into account the case where the source is in the local lattice
...
static void threep_oneD_host(ProfileStruct &ps, Float2<FloatC> *result, PLEGMA_Correlator<FloatC> &corr,
  ...
  int extra=N_DIMS*(isZfac ? (N_SPINS*N_SPINS*N_COLS*N_COLS):1);
  ...
  for(int it=0; it < t_size; it+=time_step) {
    ...
    for(int et=0; et < extra; et++) {
      if(isZfac) {
	int mu=et/N_SPINS/N_COLS/N_COLS/N_DIMS;
	int nu=(et/N_COLS/N_COLS/N_DIMS)%N_SPINS;
	int c1=(et/N_COLS/N_DIMS)%N_COLS;
	int c2=(et/N_DIMS)%N_COLS;
	int dir=et%N_DIMS;
	threep_oneD_zfac_device<FloatC,FloatA, FloatB, FloatG>
	  <<<grid,ps.tp.block,ps.tp.shared_bytes>>>
	  (d_partial_block, *propTex1, *propTex2, *gaugetex, listGammas, it, t_step, maxT,
	   source, signProps, runFT, *moms, mu,nu,c1,c2, dir);
      }	else {
	int dir=et%N_DIMS;
	threep_oneD_device<FloatC,FloatA, FloatB, FloatG>
	  <<<grid,ps.tp.block,ps.tp.shared_bytes>>>
	  (d_partial_block, *propTex1, *propTex2, *gaugetex, listGammas, it, t_step, maxT,
	   source, signProps, runFT, *moms, dir);
      }
...
void threep_oneD(PLEGMA_Correlator<FloatC> &corr, PLEGMA_Propagator<FloatA>& prop1, PLEGMA_Propagator<FloatB>& prop2,
  ...
  tune( ps, "threep_oneD", threep_oneD_host<FloatC,FloatA,FloatB,FloatG>,
	ps, result, corr, prop1, prop2, signProps, gauge, gammas, false);
  run( ps, "threep_oneD", threep_oneD_host<FloatC,FloatA,FloatB,FloatG>,
