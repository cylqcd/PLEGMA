...
__global__ void threep_oneD_device(Float2<FloatC>* block2,
  ...
  if (sid3D < DGC->localVolume3D){
    ...
    for(int dir = 0; dir < N_DIMS; dir++) {
      ...
      // The order has been optmized such that 3 data accesses have been commented out
      // + term x, x, x+dir
      texture1.get(prop1,vid); gaugeTex.get(su3,dir,vid); texture2.template get<Plus>(prop2,vid,dir);
      partial_trace_mul_Prop_G_Prop<true,ZERO_PLUS,false>(R,prop1,prop2,su3,mu,nu,c1,c2);

      // - term x, x-dir^, x-dir
      /*texture1.get(prop1,vid);*/ gaugeTex.template get<Minus>(su3,dir,vid,dir); texture2.template get<Minus>(prop2,vid,dir);
      partial_trace_mul_Prop_G_Prop<true,ACC_MINUS,true>(R,prop1,prop2,su3,mu,nu,c1,c2);

      // + term x-dir, x-dir, x
      texture1.template get<Minus>(prop1,vid,dir); /*gaugeTex.get<Minus>(su3,dir,vid,dir);*/ texture2.get(prop2,vid);
      partial_trace_mul_Prop_G_Prop<true,ACC_PLUS,false>(R,prop1,prop2,su3,mu,nu,c1,c2);

      // - term x+dir, x^, x
      texture1.template get<Plus>(prop1,vid,dir); gaugeTex.get(su3,dir,vid); /*texture2.get(prop2,vid);*/
      partial_trace_mul_Prop_G_Prop<true,ACC_MINUS,true>(R,prop1,prop2,su3,mu,nu,c1,c2);
	...
	if(notZfac){
	 if (signProps >0){
           accum[dir*listGammas.size + iop]=0.25*trace_gamma_S<true>(opId,TMP,R);
         }
         else if (signProps<0){
           accum[dir*listGammas.size + iop]=0.25*trace_gamma_S<true>(opId,TMM,R);
         }
         else{
           accum[dir*listGammas.size + iop]=0.25*trace_gamma_S<true>(opId,NOROT,R);
         }
        }
	else{
...
static void threep_oneD_host(ProfileStruct &ps, Float2<FloatC> *result, PLEGMA_Correlator<FloatC> &corr,
  ...
  for(int it=0; it < t_size; it+=time_step) {
	...
	c2=et%N_COLS;
      ...
      threep_oneD_device<b,FloatC,FloatA, FloatB, FloatG>
...
template<bool b,typename FloatC,typename FloatA,typename FloatB,typename FloatG>
void threep_oneD(PLEGMA_Correlator<FloatC> &corr,
  ...
  tune( ps, "threep_oneD", threep_oneD_host<b, FloatC,FloatA,FloatB,FloatG>,
	ps, result, corr, prop1, prop2, signProps, gauge, gammas, false);
  run( ps, "threep_oneD", threep_oneD_host<b, FloatC,FloatA,FloatB,FloatG>,
...
template void threep_oneD<true, float,float,float,float>(PLEGMA_Correlator<float> &corr, PLEGMA_Propagator<float>& prop1, PLEGMA_Propagator<float>& prop2, int signProps, PLEGMA_Gauge<float>& gauge, std::vector<GAMMAS>& gammas, bool isZfac);
template void threep_oneD<true, double,double,double,double>(PLEGMA_Correlator<double> &corr, PLEGMA_Propagator<double>& prop1, PLEGMA_Propagator<double>& prop2, int signProps, PLEGMA_Gauge<double>& gauge, std::vector<GAMMAS>& gammas, bool isZfac);

template void threep_oneD<false, float,float,float,float>(PLEGMA_Correlator<float> &corr, PLEGMA_Vector<float>& prop1, PLEGMA_Vector<float>& prop2, int signProps, PLEGMA_Gauge<float>& gauge, std::vector<GAMMAS>& gammas, bool isZfac);
template void threep_oneD<false, double,double,double,double>(PLEGMA_Correlator<double> &corr, PLEGMA_Vector<double>& prop1, PLEGMA_Vector<double>& prop2, int signProps, PLEGMA_Gauge<double>& gauge, std::vector<GAMMAS>& gammas, bool isZfac);
