...
__global__ void contract_mesons_open_LIBE_device( propTex<FloatA> texProp1,
  ...
  register Float2<FloatC> accum[9];
  for(int a = 0 ; a < 9; a++)
    accum[a] = 0;
...
#pragma unroll
      for(int s = 0 ; s < 9; s++){
	accum[s] = accum[s] + prop1[alpha][beta][b][s/3] * conj(prop2[delta][gamma][b][s%3]);
      }
    }
  }
  if(runFT) {
    ...
    fourier_transform_3D(block2, accum, shared_cache, 9, sid3D, source_pos, moms, 0, -1, time_step, tid);
  } else {
    if (sid3D < DGC_localVolume3D)
#pragma unroll
	for(int s = 0 ; s < 9; s++)
	  block2[(tid*DGC_localVolume3D + sid3D)*9+s] = accum[s];
  }
}

template<typename FloatA, typename FloatB, typename FloatC>
void contract_mesons_open_LIBE_host( ProfileStruct &ps,
  ...
  for(int it=0; it < t_size; it+=time_step) {
    ...
    for(int ip=0; ip < site_size; ip++) {
      contract_mesons_open_LIBE_device<FloatA, FloatB, FloatC>
	...
	for(size_t v = 0 ; v < volume*std::min(t_size-it, time_step); v++) {
	  for(int i = 0 ; i < 9; i++) {
	    result[((it*volume+v)*9 + i)*site_size+ip] = 0;
	    for(int j = 0 ; j < accumX; j++)
	      result[((it*volume+v)*9 + i)*site_size+ip] += h_partial_block[(v*9+i)*accumX+j];
	  }
	}
      } else {
	for(size_t v = 0 ; v < volume; v++)
	  for(int i = 0 ; i < 9; i++)
	    result[((it*volume+v)*9 + i)*site_size+ip] = h_partial_block[(v*9+i)];
      }
    }
  }
...
template<typename FloatA, typename FloatB, typename FloatC>
static void contract_mesons_open_LIBE(PLEGMA_Propagator<FloatA>& prop1, PLEGMA_Propagator<FloatB>& prop2,
			    PLEGMA_Correlator<FloatC>& corr){
  bool runFT = (corr.getCorrSpace()==MOMENTUM_SPACE);
  int site_size = 3*3*N_SPINS*N_SPINS*N_SPINS*N_SPINS;
  
  if(corr.getSiteSize() != site_size)
    PLEGMA_error("Correlator siteSize do not match: %d != %d\n", corr.getSiteSize(), site_size);

  int shared_size = (runFT==true) ? 9*sizeof(Float2<FloatC>) : 0;
  ...
  tuneAndRun( ps, "contract_mesons_open_LIBE", contract_mesons_open_LIBE_host<FloatA,FloatB,FloatC>,
