...
template<typename FloatC, typename FloatA, typename FloatB, typename FloatG, int COLS>
__global__ void mesons_noe_device(Float2<FloatC>* block2,
  ...
  if (sid3D < DGC_localVolume3D){
    ...
    for(int dir = 0; dir < N_DIMS; dir++) {
      ...
      partial_trace_mul_Prop_G_Prop<COLS,ACC_ZERO,false>(R,prop1,prop2,su3);
...
static void mesons_noe_host(ProfileStruct &ps, Float2<FloatC> *result, PLEGMA_Correlator<FloatC> &corr,
			    PLEGMA_Propagator<FloatA>& prop1, PLEGMA_Propagator<FloatA>& prop2,
			    PLEGMA_Gauge<FloatG>& gauge, bool all_cols){
  ...
  for(int it=0; it < t_size; it+=time_step) {
    ...
    if(all_cols)
      mesons_noe_device<FloatC,FloatA, FloatB, FloatG,N_COLS>
	<<<grid,ps.tp.block,ps.tp.shared_bytes>>>
	(d_partial_block, *propTex1, *propTex2, *gaugetex, it, t_step, maxT, source, runFT, *moms);
    else
      mesons_noe_device<FloatC,FloatA, FloatB, FloatG,1>
	<<<grid,ps.tp.block,ps.tp.shared_bytes>>>
	(d_partial_block, *propTex1, *propTex2, *gaugetex, it, t_step, maxT, source, runFT, *moms); 
...
void mesons_noe(PLEGMA_Correlator<FloatC> &corr, PLEGMA_Propagator<FloatA>& prop1, PLEGMA_Propagator<FloatB>& prop2, PLEGMA_Gauge<FloatG>& gauge, bool all_cols) {
  ...
  tuneAndRun( ps, "mesons_noe", mesons_noe_host<FloatC,FloatA,FloatB,FloatG>,
	      ps, result, corr, prop1, prop2, gauge, all_cols);
...
template void mesons_noe<float,float,float,float>(PLEGMA_Correlator<float> &corr, PLEGMA_Propagator<float>& prop1, PLEGMA_Propagator<float>& prop2, PLEGMA_Gauge<float>& gauge, bool all_cols);
template void mesons_noe<double,double,double,double>(PLEGMA_Correlator<double> &corr, PLEGMA_Propagator<double>& prop1, PLEGMA_Propagator<double>& prop2, PLEGMA_Gauge<double>& gauge, bool all_cols);
