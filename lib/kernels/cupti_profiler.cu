...
namespace detail {
  ...
  profiler::profiler(const strvec_t& events,
  ...
  void profiler::print_event_values(std::ostream& s,
				    bool print_names,
  ...
  void profiler::print_metric_values(std::ostream& s,
			   bool print_names,
			   const char* kernel_separator) {
    if(m_num_metrics <= 0)
      return;
  ...
  void profiler::print_events_and_metrics(std::ostream& s,
					  bool print_names,
					  const char* kernel_separator) {
  ...
  event_val_t
  profiler::get_event_values(const char *kernel_name) {
    if(m_num_events > 0)
  ...
  metric_val_t profiler::get_metric_values(const char *kernel_name) {
    if(m_num_metrics > 0)
