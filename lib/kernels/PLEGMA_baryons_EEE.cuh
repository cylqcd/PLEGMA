  ...
  if (sid3D < DGC_localVolume3D){
    ...
    for(int nz = 0 ; nz < N_SPINS ; nz++){
      ...
      for(int ei = 0 ; ei < 6 ; ei++){
	      int a = eps[ei][0]; //Where is this defined?
	      int b = eps[ei][1];
	      int c = eps[ei][2];
...
static void contract_baryons_EEE(Float* evecs, Float* evals, int nvecs, size_t vec_size, bool dev_ptr,
  ...
  for(int h=0; h<nvecs; h++) {
    ...
    if(dev_ptr) {
      ...
      cudaMemcpy(vec1.D_elem(), evecs+h*vec_size, vec_bytes, cudaMemcpyHostToDevice);
      checkCudaError();
    }
    ...
    for(int i=0; i<h; i++) {
      for(int k=0; k<ils and k<nvecs; k++) {
	      if(dev_ptr) {
	        ...
	        cudaMemcpyAsync(vec3[k]->D_elem(), evecs+k*vec_size, vec_bytes, cudaMemcpyHostToDevice, stream[k]);
	      }
      }
      ...
      for(int j=0; j<nvecs; j++) {
        ...
        if(j+ils<nvecs) {
          if(dev_ptr) {
            ...
            cudaMemcpyAsync(vec3[k]->D_elem(), evecs+(j+ils)*vec_size, vec_bytes, cudaMemcpyHostToDevice, stream[k]);
          }
        }
        ...
        // PLEGMA_printf("Eijk\n");
        // PLEGMA_printf("%d %d %d\n", h, i, j);
        // PLEGMA_printf("t p g re im\n");
        // for(int global_it=0; global_it<HGC_totalL[3]; global_it++){
        //   int my_it = global_it - HGC_procPosition[3] * HGC_localL[3];
        //   bool is_myIt = (my_it >= 0) && ( my_it < HGC_localL[3] );
        //   if(is_myIt){
        //     for(int v=0; v<volume; v++) {
        //       for(int is=0; is<4; is++) {
        //         PLEGMA_printf("%d %d %d %e %e\n", global_it, v, is, in[(my_it*volume+v)*4+is].real(), in[(my_it*volume+v)*4+is].imag());
        //       }
        //     }
        //   }
        // }

        std::complex<Float> bcsgn;
        std::complex<Float> eval = 1;
        eval /= (evs[h]*evs[i]*conj(evs[j]));
        // PLEGMA_printf("%d %d %d %e %e %e %e %e %e\n", h, i, j, evs[h], evs[i], evs[j]);
          ...
          for(int t1=0; t1<t_size; t1++) {
            ...
            for(int v=0; v<volume; v++) {
              for(int is=0; is<site_size/2; is++) {
                complex<Float> tmp = bcsgn * in[(t0*volume+v)*4+is/4]*conj(in[(t0pt*volume+v)*4+(is+2)%4]); // is+2 to account for contraction with gamma_5
