...
template<typename FloatA, typename FloatC>
void contract_tetraquarks_host(ProfileStruct &ps,
...
//  cudaMalloc((void**)&vals, size*sizeof(Float2<float>));
  ...
  for(int j=0; j<TETRA_prop_prods_count[i].size(); j++) {
    ...
    cudaMemcpy(col_contr+8*shift, TETRA_prop_prods_col_contr[i][j], 8*TETRA_prop_prods_count[i][j]*sizeof(short), cudaMemcpyHostToDevice);
    cudaMemcpy(vals+shift, TETRA_prop_prods_vals[i][j], TETRA_prop_prods_count[i][j]*sizeof(Float2<float>), cudaMemcpyHostToDevice);
  ...
  for(int it=0; it < t_size; it+=time_step) {
	...
	contract_props
	  ...
	  (*propTex1, *propTex2, *propTex3, *propTex4, d_partial_block, TETRA_prop_prods_count[i][j], idxs+8*shift, col_contr+8*shift, vals+shift,
...
void contract_tetraquarks(PLEGMA_Propagator<FloatA>& propLT, PLEGMA_Propagator<FloatA>& propST, 
  ...
  for(auto i: todo) {
    ...
    if(runFT)
      hostMalloc(result, TETRA_prop_prods_count[i].size()*corr.getVolSize()*sizeof(Float2<FloatC>));
    else
      result = ((Float2<FloatC> *) corr.H_elem()) + shift*corr.getVolSize();

    PLEGMA_Propagator<FloatA>* props[4];
    for (int j=0; j<4; j++) {
      if(TETRA_prop_prods[i][j] == 'u')
	props[j] = &propLT;
      else if(TETRA_prop_prods[i][j] == 'd')
	props[j] = &propLT;
      else if(TETRA_prop_prods[i][j] == 's')
	props[j] = &propST;
      else if(TETRA_prop_prods[i][j] == 'c')
	props[j] = &propCH;
      else if(TETRA_prop_prods[i][j] == 'b')
	props[j] = &propBT;
      else
	PLEGMA_error("Unknown propagator %c", TETRA_prop_prods[i][j]);
    }
    ...
    if(HGC_verbosity>2) PLEGMA_printf("Running for %s\n", TETRA_prop_prods[i].c_str());
    tuneAndRun(ps, "contract_tetraquarks_size"+std::to_string(TETRA_prop_prods_count[i].size()), contract_tetraquarks_host<FloatA,FloatC>, ps, props, corr, result, i);

    if(runFT) {
      FloatC *corr_ip = corr.H_elem() + shift*corr.getVolSize()*2;
      MPI_Allreduce(result, corr_ip, TETRA_prop_prods_count[i].size()*corr.getVolSize()*2, MPI_Type(corr_ip),
		    MPI_SUM, HGC_spaceComm);
      hostFree(result, TETRA_prop_prods_count[i].size()*corr.getVolSize()*sizeof(Float2<FloatC>));
    }
    shift+=TETRA_prop_prods_count[i].size();
  }
}

template
void contract_tetraquarks<float,double>(PLEGMA_Propagator<float>& propLT, PLEGMA_Propagator<float>& propST, 
                                       PLEGMA_Propagator<float>& propCH, PLEGMA_Propagator<float>& propBT,
					PLEGMA_Correlator<double> &corr, std::vector<int> &todo);

template
void contract_tetraquarks<float,float>(PLEGMA_Propagator<float>& propLT, PLEGMA_Propagator<float>& propST, 
                                       PLEGMA_Propagator<float>& propCH, PLEGMA_Propagator<float>& propBT,
					PLEGMA_Correlator<float> &corr, std::vector<int> &todo);

template
void contract_tetraquarks<double,double>(PLEGMA_Propagator<double>& propLT, PLEGMA_Propagator<double>& propST, 
