...
#pragma unroll
  ...
  template<typename Float>
  __inline__ __device__ void fourier_transform_3D( Float2<Float> *out, Float2<Float> *in, Float2<Float> *shared_cache, int n_comp, int sid3D, int sp[3], int sign = -1, int padding = 0, int shift = 0){
	...
	for(int ip = 0 ; ip < n_comp ; ip++){
	  out[(imom*(n_comp+padding) + shift + ip)*gridDim.x + blockIdx.x] = shared_cache[ip*blockDim.x];
	}
      }
    }    
  }
