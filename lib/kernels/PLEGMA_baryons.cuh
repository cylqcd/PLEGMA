...
template<typename FloatA, typename FloatB, typename FloatC>
__global__ void contract_baryons_wall_device(propTex<FloatA> texProp1, propTex<FloatB> texProp2, propTex<FloatA> texProp3, propTex<FloatB> texProp4, Float2<FloatC>* block2,
                                             int it, int time_step, int maxT, int4 source, BARYONS_TYPE ip, bool runFT, tex_mom_list mom_list){
...
template<typename FloatA, typename FloatB, typename FloatC>
static void contract_baryons_wall_host( ProfileStruct &ps,
                                        PLEGMA_Propagator<FloatA>& prop1, PLEGMA_Propagator<FloatB>& prop2, PLEGMA_Propagator<FloatA>& prop3, PLEGMA_Propagator<FloatB>& prop4,
					PLEGMA_Correlator<FloatC> &corr, Float2<FloatC> *result, int ip){
...
static void contract_baryons_wall(PLEGMA_Propagator<FloatA>& prop1, PLEGMA_Propagator<FloatB>& prop2, PLEGMA_Propagator<FloatA>& prop3, PLEGMA_Propagator<FloatB>& prop4, PLEGMA_Correlator<FloatC> &corr){
  ...
  for(int ip=0; ip<N_BARYONS; ip++) {
    std::string name = "contract_baryons_wall_"+std::to_string(ip);
    tuneAndRun( ps, name, contract_baryons_wall_host<FloatA,FloatB,FloatC>,
	        ps, prop1, prop2, prop3, prop4, corr, result, ip);
    if(runFT) {
      FloatC *corr_ip = corr.H_elem() + ip*corr.getTotalSize()/N_BARYONS*2;
      MPI_Allreduce(result, corr_ip, corr.getTotalSize()/N_BARYONS*2, MPI_Type(corr_ip),
      MPI_SUM, HGC_spaceComm);
