...
template<bool CONJ_P,unsigned int C1, unsigned int C2, unsigned int NG,typename FloatOut, typename FloatV, typename FloatP>
void V_kernels( ProfileStruct &ps, VRED V, Float2<FloatOut> *block2,
  ...
  if(V==V_2)
    V2_kernel<FloatOut,FloatV,FloatP,NG,CONJ_P><<<ps.tp.grid,ps.tp.block,ps.tp.shared_bytes>>>(Phi1, listGammas, S1, S2, block2, it, time_step, maxT, source, moms);
  else if(V==V_3)
    V3_kernel<FloatOut,FloatV,FloatP,NG,CONJ_P><<<ps.tp.grid,ps.tp.block,ps.tp.shared_bytes>>>(Phi1, listGammas, S1, block2, it, time_step, maxT, source, moms);
  else if(V==V_4)
    V4_kernel<FloatOut,FloatV,FloatP,NG,CONJ_P><<<ps.tp.grid,ps.tp.block,ps.tp.shared_bytes>>>(Phi1, listGammas, S1, S2, block2, it, time_step, maxT, source, moms);
  else if(V==V_6_RED)
    V6_RED_kernel<FloatOut,FloatV,FloatP,NG,CONJ_P,C1,C2><<<ps.tp.grid,ps.tp.block,ps.tp.shared_bytes>>>(Phi1, Phi2, listGammas, S1, block2, it, time_step, maxT, source, moms);
  else 
    PLEGMA_error("Unrecognized V reduction type\n");
}

template<bool CONJ_P,unsigned int NG, typename FloatOut, typename FloatV, typename FloatP>
void V_kernels_nogamma( ProfileStruct &ps, VRED V, Float2<FloatOut> *block2,
  ...
  if(V==V_5){
    V5_kernel<FloatOut,FloatV,FloatP,NG,CONJ_P><<<ps.tp.grid,ps.tp.block,ps.tp.shared_bytes>>>(Phi1, Phi2,  block2, it, time_step, maxT, source, moms);
  }
  else if(V==V_6){
    V6_kernel<FloatOut,FloatV,FloatP,NG,CONJ_P><<<ps.tp.grid,ps.tp.block,ps.tp.shared_bytes>>>(Phi1, Phi2,  S1, block2, it, time_step, maxT, source, moms);
  }
  else
    PLEGMA_error("Unrecognized V reduction type\n");
}
...
void V_kernels_wrapper( ProfileStruct &ps, VRED V, Float2<FloatOut> *block2,
  ...
  case(0): V_kernels_nogamma<CONJ_P,(unsigned int)0,FloatOut,FloatV,FloatP>( ps, V, block2, it, time_step, maxT, source, moms, Phi1, Phi2,S1 ); break;
  case(1): V_kernels<CONJ_P,C1,C2,(unsigned int)1,FloatOut,FloatV,FloatP>( ps, V, block2, it, time_step, maxT, source, moms, listGammas, Phi1, Phi2,S1, S2 ); break;
  case(3): V_kernels<CONJ_P,C1,C2,(unsigned int)3,FloatOut,FloatV,FloatP>( ps, V, block2, it, time_step, maxT, source, moms, listGammas, Phi1, Phi2,S1, S2 ); break;
