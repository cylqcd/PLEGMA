...
__global__ void contract_props_stochastic(propTex<FloatA> texProp1, propTex<FloatA> texProp2, propTex<FloatA> texProp3,
...
template<typename FloatA, typename FloatC>
void contract_tetraquarks_stochastic_host(ProfileStruct &ps,
  ...
  Float2<float> *vals;
  int size = 0;
  for(int j=0; j<TETRA_stoch_prop_prods_count[i].size(); j++)
    size += TETRA_stoch_prop_prods_count[i][j];
  ...
  int shift = 0;
  for(int j=0; j<TETRA_stoch_prop_prods_count[i].size(); j++) {
    qudaMemcpy(idxs+8*shift, TETRA_stoch_prop_prods_idxs[i][j], 8*TETRA_stoch_prop_prods_count[i][j]*sizeof(short), qudaMemcpyHostToDevice);
    qudaMemcpy(col_contr+8*shift, TETRA_stoch_prop_prods_col_contr[i][j], 8*TETRA_stoch_prop_prods_count[i][j]*sizeof(short), qudaMemcpyHostToDevice);
    qudaMemcpy(vals+shift, TETRA_stoch_prop_prods_vals[i][j], TETRA_stoch_prop_prods_count[i][j]*sizeof(Float2<float>), qudaMemcpyHostToDevice);
    shift += TETRA_stoch_prop_prods_count[i][j];
  }
  ...
  for(int it=0; it < t_size; it+=time_step) {
	...
	contract_props_stochastic
	  <<<grid,ps.tp.block,ps.tp.shared_bytes>>>
	  (*propTex1, *propTex2, *propTex3, *propTex4, *propTex5, *propTex6, *propTex7, *propTex8, d_partial_block, TETRA_stoch_prop_prods_count[i][j], idxs+8*shift, col_contr+8*shift, vals+shift,
...
template<typename FloatA, typename FloatC>
void contract_tetraquarks_stochastic(PLEGMA_Propagator<FloatA>&propLT1, PLEGMA_Propagator<FloatA>&propLT2, 
  ...
  for(auto i: todo) {
    ...
    if(runFT)
      hostMalloc(result, TETRA_stoch_prop_prods_count[i].size()*corr.getVolSize()*sizeof(Float2<FloatC>));
    else
...
//     props[3]=&propBT2;
    ...
    for (int j=0; j<4; j++) {
      if(TETRA_stoch_prop_prods[i][j] == 'u'){
	props[2*j] = &propLT1;
    props[2*j+1] = &propLT2;}
      else if(TETRA_stoch_prop_prods[i][j] == 'd'){
    props[2*j] = &propLT1;
    props[2*j+1] = &propLT2;}
      else if(TETRA_stoch_prop_prods[i][j] == 's'){
    props[2*j] = &propST1;
    props[2*j+1] = &propST2;}
//        else if(TETRA_stoch_prop_prods[i][j] == 'c'){
//  	props[j] = &propCH;}
      else if(TETRA_stoch_prop_prods[i][j] == 'b'){
	props[2*j] = &propBT1;
    props[2*j+1] = &propBT2;}
      else{
...
template
void contract_tetraquarks_stochastic<float,double>(PLEGMA_Propagator<float>&propLT1, PLEGMA_Propagator<float>&propLT2, 
...
template
void contract_tetraquarks_stochastic<float,float>(PLEGMA_Propagator<float>&propLT1, PLEGMA_Propagator<float>&propLT2, 
                                                  PLEGMA_Propagator<float>&propST1, PLEGMA_Propagator<float>&propST2,
                                       PLEGMA_Propagator<float>&propBT1, PLEGMA_Propagator<float>&propBT2,
					PLEGMA_Correlator<float> &corr, std::vector<int> &todo);

template
void contract_tetraquarks_stochastic<double,double>(PLEGMA_Propagator<double>&propLT1, PLEGMA_Propagator<double>&propLT2, 
