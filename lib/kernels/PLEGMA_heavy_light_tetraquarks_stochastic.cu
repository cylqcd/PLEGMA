...
void contract_tetraquarks_stochastic_host(ProfileStruct &ps,
  ...
  int shift = 0;
  for(int j=0; j<TETRA_stoch_prop_prods_count[i].size(); j++) {
    cudaMemcpy(idxs+8*shift, TETRA_stoch_prop_prods_idxs[i][j], 8*TETRA_stoch_prop_prods_count[i][j]*sizeof(short), cudaMemcpyHostToDevice);
    cudaMemcpy(col_contr+8*shift, TETRA_stoch_prop_prods_col_contr[i][j], 8*TETRA_stoch_prop_prods_count[i][j]*sizeof(short), cudaMemcpyHostToDevice);
    cudaMemcpy(vals+shift, TETRA_stoch_prop_prods_vals[i][j], TETRA_stoch_prop_prods_count[i][j]*sizeof(Float2<float>), cudaMemcpyHostToDevice);
