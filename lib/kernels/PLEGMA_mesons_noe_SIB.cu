...
__global__ void mesons_noe_SIB_device(Float2<FloatC>* block2,
  ...
  if (sid3D < DGC_localVolume3D){
    ...
    Float2<FloatC> R[5][N_SPINS][N_SPINS];
    ...
    for(int dir = 0; dir < N_DIMS; dir++) {
      ...
      partial_trace_mul_Prop_G_Prop_meson_SIB<ACC_ZERO,false>(R,prop1,prop2,su3);
      for(int s=0; s<5; s++){
	noeV[s*N_DIMS+dir] += trace_gamma_S<true>(gamma_dir[dir],NOROT,R[s]) - trace_gamma_S<true>(G5,NOROT,R[s]);
      }

      //term x, x-dir, x-dir
      gaugeTex.get<Minus>(su3,dir,vid,dir); prop2Tex.get<Minus>(prop2,vid,dir);
      partial_trace_mul_Prop_G_Prop_meson_SIB<ACC_ZERO,true>(R,prop1,prop2,su3);
      for(int s=0; s<5; s++){
	noeV[s*N_DIMS+dir] += trace_gamma_S<true>(gamma_dir[dir],NOROT,R[s]) - trace_gamma_S<true>(G5,NOROT,R[s]);
      }
...
template<typename FloatC,typename FloatA, typename FloatB, typename FloatG>
static void mesons_noe_SIB_host(ProfileStruct &ps, Float2<FloatC> *result, PLEGMA_Correlator<FloatC> &corr,
  ...
  for(int it=0; it < t_size; it+=time_step) {
    ...
    mesons_noe_SIB_device<FloatC,FloatA, FloatB, FloatG>
...
void mesons_noe_SIB(PLEGMA_Correlator<FloatC> &corr, PLEGMA_Propagator<FloatA>& prop1, PLEGMA_Propagator<FloatB>& prop2, PLEGMA_Gauge<FloatG>& gauge) {
  ...
  tuneAndRun( ps, "mesons_noe_SIB", mesons_noe_SIB_host<FloatC,FloatA,FloatB,FloatG>,
...
template void mesons_noe_SIB<float,float,float,float>(PLEGMA_Correlator<float> &corr, PLEGMA_Propagator<float>& prop1, PLEGMA_Propagator<float>& prop2, PLEGMA_Gauge<float>& gauge);
template void mesons_noe_SIB<double,double,double,double>(PLEGMA_Correlator<double> &corr, PLEGMA_Propagator<double>& prop1, PLEGMA_Propagator<double>& prop2, PLEGMA_Gauge<double>& gauge);
