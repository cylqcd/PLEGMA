...
__global__ void contract_mesons_open_SIR_device( propTex<FloatA> texProp1,
  ...
  register Float2<FloatC> accum[3];
  for(int a = 0 ; a < 3; a++)
    accum[a] = 0;
...
#pragma unroll
	...
	int s1 = ((s+1) / 2); // 0, 1, 1
	int s2 = ((s+0) / 2); // 0, 0, 1
	accum[s] = accum[s] + prop1[alpha][beta][b][s1] * conj(prop2[delta][gamma][b][s2]);
      }
    }
  }
  if(runFT) {
    ...
    fourier_transform_3D(block2, accum, shared_cache, 3, sid3D, source_pos, moms, 0, -1, time_step, tid);
  } else {
    if (sid3D < DGC_localVolume3D)
#pragma unroll
	for(int s = 0 ; s < 3; s++)
	  block2[(tid*DGC_localVolume3D + sid3D)*3+s] = accum[s];
  }
}

template<typename FloatA, typename FloatB, typename FloatC>
void contract_mesons_open_SIR_host( ProfileStruct &ps,
  ...
  for(int it=0; it < t_size; it+=time_step) {
    ...
    for(int ip=0; ip < site_size; ip++) {
      contract_mesons_open_SIR_device<FloatA, FloatB, FloatC>
	...
	for(size_t v = 0 ; v < volume*std::min(t_size-it, time_step); v++) {
	  for(int i = 0 ; i < 3; i++) {
	    result[((it*volume+v)*3 + i)*site_size+ip] = 0;
	    for(int j = 0 ; j < accumX; j++)
	      result[((it*volume+v)*3 + i)*site_size+ip] += h_partial_block[(v*3+i)*accumX+j];
	  }
	}
      } else {
	for(size_t v = 0 ; v < volume; v++)
	  for(int i = 0 ; i < 3; i++)
	    result[((it*volume+v)*3 + i)*site_size+ip] = h_partial_block[(v*3+i)];
      }
    }
  }
...
template<typename FloatA, typename FloatB, typename FloatC>
static void contract_mesons_open_SIR(PLEGMA_Propagator<FloatA>& prop1, PLEGMA_Propagator<FloatB>& prop2,
			    PLEGMA_Correlator<FloatC>& corr){
  bool runFT = (corr.getCorrSpace()==MOMENTUM_SPACE);
  int site_size = 3*N_SPINS*N_SPINS*N_SPINS*N_SPINS;
  
  if(corr.getSiteSize() != site_size)
    PLEGMA_error("Correlator siteSize do not match: %d != %d\n", corr.getSiteSize(), site_size);

  int shared_size = (runFT==true) ? 3*sizeof(Float2<FloatC>) : 0;
  ...
  tuneAndRun( ps, "contract_mesons_open_SIR", contract_mesons_open_SIR_host<FloatA,FloatB,FloatC>,
