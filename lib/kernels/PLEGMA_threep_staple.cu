...
__global__ void threep_staple_device(Float2<FloatC>* block2,
...
template<typename FloatC,typename FloatA, typename FloatB, typename FloatS>
static void threep_staple_host(ProfileStruct &ps, Float2<FloatC> *result,
...
//  cudaMalloc((void**)&d_partial_block, alloc_size * sizeof(Float2<FloatC>) );
  ...
  for(int it=0; it < t_size; it+=time_step) {
	...
	c2=et%N_COLS;
      ...
      threep_staple_device<FloatC,FloatA,FloatB,FloatS>
...
void threep_staple(PLEGMA_Correlator<FloatC> &corr,
  ...
  tune( ps, "threep_staple", threep_staple_host<FloatC,FloatA,FloatB,FloatS>,
	ps, result, corr, prop1, prop2, signProps, su3, gammas, false);
  run( ps, "threep_staple", threep_staple_host<FloatC,FloatA,FloatB,FloatS>,
       ps, result, corr, prop1, prop2, signProps, su3, gammas, isZfac);
...
template void threep_staple<float,float,float,float>(PLEGMA_Correlator<float> &corr, PLEGMA_Propagator<float>& prop1, PLEGMA_Propagator<float>& prop2, int signProps, PLEGMA_Su3field<float>& su3, std::vector<GAMMAS>& gammas,bool isZfac);
template void threep_staple<double,double,double,double>(PLEGMA_Correlator<double> &corr, PLEGMA_Propagator<double>& prop1, PLEGMA_Propagator<double>& prop2, int signProps, PLEGMA_Su3field<double>& su3, std::vector<GAMMAS>& gammas,bool isZfac);
