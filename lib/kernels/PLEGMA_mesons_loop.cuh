...
__global__ void contract_loop_device( vectorTex<Float> vect,
				      propTex<Float> prop,
				      Float2<Float> *block2,
  ...
  register Float2<Float> accum[N_SPINS*N_SPINS];
  ...
  if (sid3D < DGC_localVolume3D){
    ...
    Float2<Float> prop1[N_SPINS][N_SPINS][N_COLS][1];

    vect.get(vect1,vid);
    prop.get<1>(prop1,vid);
#pragma unroll
...
#pragma unroll
	for(int a = 0 ; a < N_COLS ; a++){
	  accum[is] = accum[is] + val * conj(vect1[nu][a]) * prop1[mu][nu][a][0];
	}
      }
    }
  }
  if(runFT) {
    ...
    fourier_transform_3D(block2, accum, shared_cache, N_SPINS*N_SPINS, sid3D, source_pos, moms, 0, -1, time_step, tid);
  } else {
    if (sid3D < DGC_localVolume3D)
      for(int ip = 0 ; ip < N_SPINS*N_SPINS ; ip++){
	block2[(tid*DGC_localVolume3D + sid3D)*N_SPINS*N_SPINS + ip] = accum[ip];
      }
  }
}

template<typename Float>
void contract_loop_host( ProfileStruct &ps,
...
template<typename Float>
static void contract_loop(PLEGMA_Vector3D<Float>& vect, PLEGMA_Propagator3D<Float>& prop,
  ...
  tuneAndRun( ps, "contract_loop", contract_loop_host<Float>,
