  ...
  if (sid3D < DGC_localVolume3D){
	...
	for(int dir3 = 0; dir3 < N_DIMS; dir3++) {
	  ...
	  for(int iop = 0; iop < listGammas.size; iop++){
	    ...
	    if(notZfac) accum[dir_index*listGammas.size + iop] = 0.015625*((signProps > 0) ? trace_gamma_S<true>(opId,TMP,R) : trace_gamma_S<true>(opId,TMM,R));
	    else accum[dir_index*listGammas.size + iop] = trace_gamma_S<true>(opId,NOROT,R);
	  }
	  dir_index++;
	}
      }
  ...
  if(isZfac){
	...
	for(int c1 = 0 ; c1 < N_COLS; c1++)
	  for(int c2 = 0 ; c2 < N_COLS; c2++){
	    rmove = result +
	      ((mu*N_SPINS+nu)*N_COLS*N_COLS + c1*N_COLS+c2)*corr.getVolSize()
	      *N_DIMS*(N_DIMS-1)*gammas.size();
	    tuneAndRun( ps, "threep_threeD_Zfac", threep_threeD_host<FloatC,FloatA,FloatB,FloatG>,
...
template void threep_threeD<float,float,float,float>(PLEGMA_Correlator<float> &corr, PLEGMA_Propagator<float>& prop1, PLEGMA_Propagator<float>& prop2, int signProps, PLEGMA_Gauge<float>& gauge, std::vector<GAMMAS>& gammas, bool isZfac);
template void threep_threeD<double,double,double,double>(PLEGMA_Correlator<double> &corr, PLEGMA_Propagator<double>& prop1, PLEGMA_Propagator<double>& prop2, int signProps, PLEGMA_Gauge<double>& gauge, std::vector<GAMMAS>& gammas, bool isZfac);
