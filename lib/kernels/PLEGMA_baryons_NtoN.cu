...
__device__ void contract_NtoN_wall_kernel(propTex<FloatA>& texProp1, propTex<FloatB>& texProp2, propTex<FloatA>& texProp3, propTex<FloatB>& texProp4, Float2<FloatC> accum[2*N_SPINS*N_SPINS], int vid) {
  ...
  Float2<FloatB> prop4[N_SPINS][N_SPINS][N_COLS][N_COLS];
...
#pragma unroll
	  for(int cc2 = 0 ; cc2 < 6 ; cc2++){
	    ...
	    tmp[0] = tmp[0] + factor * prop2[lu][lup][b][b1] *
	      (prop1[nu][nup][a][a1] * prop1[mu][mup][c][c1] - prop1[nu][mup][a][c1] * prop1[mu][nup][c][a1]);
	    tmp[1] = tmp[1] + factor * prop4[lu][lup][b][b1] *
	      (prop3[nu][nup][a][a1] * prop3[mu][mup][c][c1] - prop3[nu][mup][a][c1] * prop3[mu][nup][c][a1]);
	  }
	}
      }
#pragma unroll
...
template __device__ void contract_NtoN_wall_kernel<float,float,float>(propTex<float>& texProp1, propTex<float>& texProp2, propTex<float>& texProp3, propTex<float>& texProp4, Float2<float> accum[2*N_SPINS*N_SPINS], int vid);
template __device__ void contract_NtoN_wall_kernel<double,double,double>(propTex<double>& texProp1, propTex<double>& texProp2, propTex<double>& texProp3, propTex<double>& texProp4, Float2<double> accum[2*N_SPINS*N_SPINS], int vid);
