...
__global__ void threep_twoD_device(Float2<FloatC>* block2,
  ...
  if (sid3D < DGC_localVolume3D){
	...
	for(int iop = 0; iop < listGammas.size; iop++){
	  ...
	  if(notZfac) accum[dir_index*listGammas.size + iop] = 0.0625*((signProps > 0) ? trace_gamma_S<true>(opId,TMP,R) : trace_gamma_S<true>(opId,TMM,R));
  ...
  if(isZfac){
	...
	for(int c1 = 0 ; c1 < N_COLS; c1++)
	  for(int c2 = 0 ; c2 < N_COLS; c2++){
	    ...
	    tuneAndRun( ps, "threep_twoD_Zfac", threep_twoD_host<FloatC,FloatA,FloatB,FloatG>,
...
template void threep_twoD<float,float,float,float>(PLEGMA_Correlator<float> &corr, PLEGMA_Propagator<float>& prop1, PLEGMA_Propagator<float>& prop2, int signProps, PLEGMA_Gauge<float>& gauge, std::vector<GAMMAS>& gammas, bool isZfac);
template void threep_twoD<double,double,double,double>(PLEGMA_Correlator<double> &corr, PLEGMA_Propagator<double>& prop1, PLEGMA_Propagator<double>& prop2, int signProps, PLEGMA_Gauge<double>& gauge, std::vector<GAMMAS>& gammas, bool isZfac);
