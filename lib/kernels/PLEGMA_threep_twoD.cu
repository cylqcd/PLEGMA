...
__global__ void threep_twoD_device(Float2<FloatC>* block2,
  ...
  if (sid3D < DGC_localVolume3D){
	...
	partial_trace_mul_Prop_G1_G2_Prop<true,ACC_PLUS,true,true>(R,prop1,prop2,su3_1,su3_2);
	...
	partial_trace_mul_Prop_G1_G2_Prop<true,ACC_MINUS,false,true>(R,prop1,prop2,su3_1,su3_2);
	...
	partial_trace_mul_Prop_G1_G2_Prop<true,ACC_PLUS,false,false>(R,prop1,prop2,su3_1,su3_2);
	...
	for(int iop = 0; iop < listGammas.size; iop++){
	  ...
	  accum[dir_index*listGammas.size + iop] = 0.0625*((signProps > 0) ? trace_gamma_S<true>(opId,TMP,R) : trace_gamma_S<true>(opId,TMM,R));
	}
	dir_index++;
      }
    }
...
template<typename FloatC,typename FloatA, typename FloatB, typename FloatG>
static void threep_twoD_host(ProfileStruct &ps, Float2<FloatC> *result, PLEGMA_Correlator<FloatC> &corr,
  ...
  for(int it=0; it < t_size; it+=time_step) {
    ...
    threep_twoD_device<FloatC,FloatA, FloatB, FloatG>
...
template<typename FloatC,typename FloatA,typename FloatB,typename FloatG>
void threep_twoD(PLEGMA_Correlator<FloatC> &corr, PLEGMA_Propagator<FloatA>& prop1, PLEGMA_Propagator<FloatB>& prop2,
		 int signProps, PLEGMA_Gauge<FloatG>& gauge, std::vector<GAMMAS>& gammas){
...
template void threep_twoD<float,float,float,float>(PLEGMA_Correlator<float> &corr, PLEGMA_Propagator<float>& prop1, PLEGMA_Propagator<float>& prop2, int signProps, PLEGMA_Gauge<float>& gauge, std::vector<GAMMAS>& gammas);
template void threep_twoD<double,double,double,double>(PLEGMA_Correlator<double> &corr, PLEGMA_Propagator<double>& prop1, PLEGMA_Propagator<double>& prop2, int signProps, PLEGMA_Gauge<double>& gauge, std::vector<GAMMAS>& gammas);
