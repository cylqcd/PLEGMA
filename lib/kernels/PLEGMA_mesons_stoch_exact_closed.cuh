...
__global__ void contract_stoch_exact_closed_device( propTex<Float> texProp,
  ...
  if (sid3D < DGC_localVolume3D){
    ...
    texVec.get(vec,vid);
    #pragma unroll
    for(int nz1 = 0 ; nz1 < N_SPINS ; nz1++){
      int beta = gammaInd[is][nz1][0];
      int gamma = gammaInd[is][nz1][1];
      Float2<Float> val1 = g[is][nz1];
      #pragma unroll
      for(int nz2 = 0 ; nz2 < N_SPINS ; nz2++){
	int delta = gammaInd[is][nz2][0];
	int alpha = gammaInd[is][nz2][1];
	Float2<Float> val2 = g[is][nz2];
        #pragma unroll
	for(int b = 0 ; b < N_COLS ; b++)
	  accum = accum + val1*val2*vec[alpha][b] * spin.vals[beta] * conj(prop[delta][gamma][b][0]);
      }
    }
  }
  if(runFT) {
...
template<typename Float>
void contract_stoch_exact_closed_host( ProfileStruct &ps,
  ...
  for(int it=0; it < t_size; it+=time_step) {
    ...
    for(int ip=0; ip < site_size; ip++) {
      contract_stoch_exact_closed_device<Float>
...
template<typename Float>
static void contract_stoch_exact_closed(PLEGMA_Propagator<Float> &prop, Float *spinVals,
  ...
  for(int k=0; k<ils and k<nvecs; k++) {
    ...
    if(dev_ptr) {
      ...
      qudaMemcpyAsync(vec[k]->D_elem(), ptr+k*vec_size, vec_bytes, qudaMemcpyHostToDevice, stream[k]);
    }
  }
  ...
  for(int i=0; i<nvecs; i++) {
    ...
    tuneAndRun( ps, "contract_stoch_exact_closed", contract_stoch_exact_closed_host<Float>,
	...
	qudaMemcpyAsync(vec[k]->D_elem(), ptr+(i+ils)*vec_size, vec_bytes, qudaMemcpyHostToDevice, stream[k]);
      }
    }
  }
