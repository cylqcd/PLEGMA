...
template<typename Float>
void PLEGMA_Propagator<Float>::ghostToHost(){   
  // direction x 
  if( GK_localL[0] < GK_totalL[0]){
    ...
    for(int c2 = 0 ; c2 < N_COLS ; c2++){
      ...
      h_elem_offset = (PLEGMA_Field<Float>::h_elem + 
		       GK_minusGhost[0]*N_SPINS*N_SPINS*
		       N_COLS*N_COLS*2 + 
		       mu*N_SPINS*N_COLS*N_COLS*GK_surface3D[0]*2 + 
		       nu*N_COLS*N_COLS*GK_surface3D[0]*2 + 
    ...
    for(int c2 = 0 ; c2 < N_COLS ; c2++){
      d_elem_offset = (PLEGMA_Field<Float>::d_elem + 
		       ...
		       position*2);  
      h_elem_offset = (PLEGMA_Field<Float>::h_elem + 
		       GK_plusGhost[0]*N_SPINS*N_SPINS*
		       N_COLS*N_COLS*2 + 
		       mu*N_SPINS*N_COLS*N_COLS*GK_surface3D[0]*2 + 
		       nu*N_COLS*N_COLS*GK_surface3D[0]*2 + 
		       c1*N_COLS*GK_surface3D[0]*2 + 
  ...
  if( GK_localL[1] < GK_totalL[1]){
    ...
    for(int c2 = 0 ; c2 < N_COLS ; c2++){
      d_elem_offset = (PLEGMA_Field<Float>::d_elem + 
		       ...
		       position*2);  
      h_elem_offset = (PLEGMA_Field<Float>::h_elem + 
		       GK_minusGhost[1]*N_SPINS*N_SPINS*
		       N_COLS*N_COLS*2 + 
		       mu*N_SPINS*N_COLS*N_COLS*GK_surface3D[1]*2 + 
		       nu*N_COLS*N_COLS*GK_surface3D[1]*2 + 
    ...
    for(int c2 = 0 ; c2 < N_COLS ; c2++){
      d_elem_offset = (PLEGMA_Field<Float>::d_elem + 
		       ...
		       position*2);  
      h_elem_offset = (PLEGMA_Field<Float>::h_elem + 
		       GK_plusGhost[1]*N_SPINS*N_SPINS*
		       N_COLS*N_COLS*2 + 
		       mu*N_SPINS*N_COLS*N_COLS*GK_surface3D[1]*2 + 
		       nu*N_COLS*N_COLS*GK_surface3D[1]*2 + 
		       c1*N_COLS*GK_surface3D[1]*2 + 
  ...
  if( GK_localL[2] < GK_totalL[2]){
    ...
    for(int c2 = 0 ; c2 < N_COLS ; c2++){
      d_elem_offset = (PLEGMA_Field<Float>::d_elem + 
		       ...
		       position*2);  
      h_elem_offset = (PLEGMA_Field<Float>::h_elem + 
		       GK_minusGhost[2]*N_SPINS*N_SPINS*
		       N_COLS*N_COLS*2 + 
		       mu*N_SPINS*N_COLS*N_COLS*GK_surface3D[2]*2 + 
		       nu*N_COLS*N_COLS*GK_surface3D[2]*2 + 
    ...
    for(int c2 = 0 ; c2 < N_COLS ; c2++){
      d_elem_offset = (PLEGMA_Field<Float>::d_elem + 
		       ...
		       position*2);  
      h_elem_offset = (PLEGMA_Field<Float>::h_elem + 
		       GK_plusGhost[2]*N_SPINS*N_SPINS*
		       N_COLS*N_COLS*2 + 
		       mu*N_SPINS*N_COLS*N_COLS*GK_surface3D[2]*2 + 
		       nu*N_COLS*N_COLS*GK_surface3D[2]*2 + 
		       c1*N_COLS*GK_surface3D[2]*2 + 
  ...
  if( GK_localL[3] < GK_totalL[3]){
    int position;
    int height = N_SPINS*N_SPINS*N_COLS*N_COLS;
    ...
    h_elem_offset = PLEGMA_Field<Float>::h_elem + GK_minusGhost[3]*N_SPINS*N_SPINS*N_COLS*N_COLS*2;
    ...
    position = 0;
    d_elem_offset = PLEGMA_Field<Float>::d_elem + position*2;
    h_elem_offset = PLEGMA_Field<Float>::h_elem + GK_plusGhost[3]*N_SPINS*N_SPINS*N_COLS*N_COLS*2;
    cudaMemcpy2D(h_elem_offset,dpitch,d_elem_offset,spitch,
		 width,height,cudaMemcpyDeviceToHost);
...
template<typename Float>
void PLEGMA_Propagator<Float>::cpuExchangeGhost(){
  if( comm_size() > 1 ){
	...
	size_t nbytes = GK_surface3D[idim]*N_SPINS*N_COLS*N_SPINS*N_COLS*2*sizeof(Float);
	// send to plus
	pointer_receive = PLEGMA_Field<Float>::h_ext_ghost + (GK_minusGhost[idim]-GK_localVolume)*N_SPINS*N_COLS*N_SPINS*N_COLS*2;
	pointer_send = PLEGMA_Field<Float>::h_elem + GK_minusGhost[idim]*N_SPINS*N_COLS*N_SPINS*N_COLS*2;
	...
	// send to minus
	pointer_receive = PLEGMA_Field<Float>::h_ext_ghost + (GK_plusGhost[idim]-GK_localVolume)*N_SPINS*N_COLS*N_SPINS*N_COLS*2;
	pointer_send = PLEGMA_Field<Float>::h_elem + GK_plusGhost[idim]*N_SPINS*N_COLS*N_SPINS*N_COLS*2;
...
template<typename Float>
void PLEGMA_Propagator<Float>::ghostToDevice(){ 
  if(comm_size() > 1){
    Float *host = PLEGMA_Field<Float>::h_ext_ghost;
    Float *device = PLEGMA_Field<Float>::d_elem + GK_localVolume*N_SPINS*N_COLS*N_SPINS*N_COLS*2;
