...
static TimeProfile profilePLEGMA("PLEGMA"); 
...
void setPreconditioners_MG(void* precUP, void* precDN){
  preconditionerUP = precUP;
  preconditionerDN = precDN;
}


void plaquette(void **gauge){
  
  profilePLEGMA.TPSTART(QUDA_PROFILE_TOTAL);  
  
  PLEGMA_Gauge<double> *pGauge = 
    new PLEGMA_Gauge<double>(BOTH,GAUGE);
  ...
  popVerbosity();
  saveTuneCache();
  profilePLEGMA.TPSTOP(QUDA_PROFILE_TOTAL);

}
#ifdef NOT_NOW
#ifdef PLEGMA_2FLAVMG
...
void calcMG_threepTwop_EvenOdd(void **gauge_APE, void **gauge, 
  ...
  //PLEGMA specific objects
  PLEGMA_Gauge<float> *K_gaugeContractions = 
    new PLEGMA_Gauge<float>(BOTH,GAUGE);
  PLEGMA_Gauge<double> *K_gaugeSmeared = 
    new PLEGMA_Gauge<double>(BOTH,GAUGE);
  PLEGMA_Vector<double> *K_vector = 
    new PLEGMA_Vector<double>(BOTH,VECTOR);
  PLEGMA_Vector<double> *K_guess = 
    new PLEGMA_Vector<double>(BOTH,VECTOR);
  PLEGMA_Vector<float> *K_temp = 
    new PLEGMA_Vector<float>(BOTH,VECTOR);

  PLEGMA_Propagator<float> *K_prop_up = 
    new PLEGMA_Propagator<float>(BOTH,PROPAGATOR);
  PLEGMA_Propagator<float> *K_prop_down = 
    new PLEGMA_Propagator<float>(BOTH,PROPAGATOR); 
  PLEGMA_Propagator<float> *K_seqProp = 
    new PLEGMA_Propagator<float>(BOTH,PROPAGATOR);

  PLEGMA_Propagator3D<float> *K_prop3D_up = 
    new PLEGMA_Propagator3D<float>(BOTH,PROPAGATOR3D);
  PLEGMA_Propagator3D<float> *K_prop3D_down = 
    new PLEGMA_Propagator3D<float>(BOTH,PROPAGATOR3D);

  PLEGMA_Correlator<float> *K_contract = 
    new PLEGMA_Correlator<float>();

  printfQuda("PLEGMA memory allocation was successfull\n");
  ...
  profilePLEGMA.TPSTART(QUDA_PROFILE_H2D);

  //PLEGMA: DMH rewite for spinor field memalloc
  ColorSpinorField *b = NULL;
  ColorSpinorField *x = NULL;
  ColorSpinorField *in = NULL;
  ColorSpinorField *out = NULL;
  
  const int *X = GK_localL;
	long int volume = X[0]*X[1]*X[2]*X[3];
  void *input_vector = malloc(volume*spinorSiteSize*sizeof(double));
  
  void *output_vector = malloc(volume*spinorSiteSize*sizeof(double));
  
  memset(input_vector,0,volume*spinorSiteSize*sizeof(double));
  memset(output_vector,0,volume*spinorSiteSize*sizeof(double));
  ...
  profilePLEGMA.TPSTOP(QUDA_PROFILE_H2D);
  ...
  for(int isource = 0 ; isource < info.Nsources ; isource++){
	...
	*( (double*)input_vector + 
      ...
      //Ensure mu is +ve
      if(param->mu < 0) param->mu *= -1.0;
      mapNormalToEvenOdd(input_vector, *param, X[0], X[1], X[2], X[3]);
      tx1 = MPI_Wtime();
      performWuppertalnStep(output_vector, input_vector, param, GK_nsmearGauss, GK_alphaGauss);
      tx2 = MPI_Wtime();
      summ_tx12 += tx2-tx1;
      mapEvenOddToNormal(output_vector, *param, X[0], X[1], X[2], X[3]);
	...
	*( (double*)input_vector + 
	...
	for(int proj=0;proj<info.Nproj[its];proj++){
	  ...
	  for(int nu = 0 ; nu < 4 ; nu++)
		...
		K_vector->copyFromQUDA(in,flag_eo);
		K_vector->download();
		K_guess->copyToQUDA(out,flag_eo); 
	  ...
	  if( CorrFileFormat==ASCII_FORM ){
	    ...
	    K_contract->copyThrpToHDF5_Buf((void*)Thrp_local_HDF5, 
					   ...
					   proj, thrp_sign, THRP_LOCAL2, 
					   CorrSpace, HighMomForm);
	    K_contract->copyThrpToHDF5_Buf((void*)Thrp_noether_HDF5, 
					   (void*)corrThp_noether, 0, 
					   uOrd, its, info.Ntsink, 
					   proj, thrp_sign, THRP_NOETHER2, 
					   CorrSpace, HighMomForm);
	    for(int mu = 0;mu<4;mu++)
	      K_contract->copyThrpToHDF5_Buf((void*)Thrp_oneD_HDF5[mu], 
					     (void*)corrThp_oneD,mu, 
					     uOrd, its, info.Ntsink, 
					     proj, thrp_sign, THRP_ONED2, 
					     CorrSpace, HighMomForm);
    ...
    K_contract->contractBaryons(*K_prop_up,*K_prop_down, corrBaryons, 
				isource, CorrSpace);
    corrMesons.contractMesons(*K_prop_up,*K_prop_down,isource, CorrSpace);
    t2 = MPI_Wtime();
    ...
    if( CorrFileFormat==ASCII_FORM ){
      ...
      K_contract->writeTwopBaryons_ASCII(corrBaryons, filename_baryons, 
					 isource, CorrSpace);
      corrMesons.writeASCII (filename_mesons);
      t2 = MPI_Wtime();
      ...
      K_contract->copyTwopBaryonsToHDF5_Buf((void*)Twop_baryons_HDF5, 
					    ...
					    CorrSpace, HighMomForm);
      corrMesons.writeHDF5(filename_mesons);
      t2 = MPI_Wtime();
...
void calcLowModeProjection(QudaInvertParam *evInvParam, 
  ...
  // PLEGMA: DMH Here the low modes of the normal M^{\dagger}M 
  ...
  PLEGMA_Deflation<double> *deflation = 
    new PLEGMA_Deflation<double>(evInvParam,arpackInfo);
  deflation->printInfo();
...
void calc_loops(void **gaugeToPlaquette, 
  ...
  if(param->dslash_type == QUDA_TWISTED_CLOVER_DSLASH ||
    ...
    QudaInvertParam tmp_param = *param;
    tmp_param.matpc_type  = QUDA_MATPC_EVEN_EVEN;
    tmp_param.dagger      = QUDA_DAG_NO;
    tmp_param.kappa       = param->kappa;
    tmp_param.mass        = 1./(2.*param->kappa) - 4.;
    tmp_param.m5          = 0.;
    tmp_param.mu          = 0.;
    tmp_param.dslash_type = (param->dslash_type == QUDA_TWISTED_CLOVER_DSLASH) ? QUDA_CLOVER_WILSON_DSLASH :  QUDA_WILSON_DSLASH;
		
    setDiracParam(dWParam, &tmp_param, false);
  }
  else{
  ...
  PLEGMA_Gauge<double> *K_gauge = 
    new PLEGMA_Gauge<double>(BOTH,GAUGE);
  K_gauge->packGauge(gaugeToPlaquette);
  K_gauge->loadGauge();
  K_gauge->calculatePlaq();
  delete K_gauge;
  // PLEGMA: DMH Calculation should default to these settings.
  ...
  // PLEGMA: DMH This is a fairly arbitrary setting in terms of
  ...
  for(int is = 0 ; is < Nrun ; is++){
	...
	if(param->inv_type == QUDA_GCR_INVERTER){
	  ...
	  Solver *solve = Solver::create(solverParam, m, mSloppy, mPre, profilePLEGMA);
	  (*solve)(*out,*in);	    
	  delete solve;
	}
