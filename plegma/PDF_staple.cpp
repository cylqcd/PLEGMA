...
int main(int argc, char **argv)
{
  ...
  size_t Zdir;
  HGC_options->set("Zdir", "Direction of the wilson line", verbosity, Zdir);
  if(Zdir>=3) PLEGMA_error("The direction of the WIlson line has to be smaller than 3");
  ...
  size_t Tmax=1;
  HGC_options->set("Tmax", "Maximum number of time separations in the staple", verbosity, Tmax);

  size_t Zmax=1;
  HGC_options->set("Zmax", "Maximum length of Wilson line", verbosity, Zmax);
  ...
  for(int ts = 0; ts < tSinks.size(); ts++) { //loop over tSinks
    ...
    // positive T
    // get the field along T direction
    su3_1->absorbDir_device(gaugeWL, Tdir);
    // initialize the line along T
    WL_1->setUnit((std::vector<int>) {0,4,8});
    // build the line along T
    for (int t = 0; t < Tmax; ++t) { // loop over T
      // if t is not zero, update the line
      if (t!=0) {
        WL_1->wilsonLineUpdate(*su3_1, tmp, 4 + Tdir);
      }
      // positive Z
      // get the field along Z direction
      su3_2->absorbDir_device(gaugeWL, Zdir);
      // initialize the line along Z
      WL_2->copy(*WL_1);
      // prepare the third line
      WL_3->copy(*WL_1);
      // build the line along Z
      for (int z = 0; z < Zmax; ++z) { // loop over Z
        // if z is not zero, update the line
        // also move the third line
        if (z!=0) {
        ...
        // build the staple
        WL1->UxUdag(*WL_2,*WL_3);
        // three point functions
        // CP1
        // get fwd prop based on the nucleon
        if(nucleon == PROTON) propF->copy(*propDN);
        else propF->copy(*propUP);
        // shift the prop along Z
        for (int zi = 0; zi < z; ++zi) {
          propExchange = propIn; propIn = propF; propF = propExchange;
          propF->shift(*propIn, 4 + Zdir);
        }
        // contract the three point function
        corrThrpWL.contractNucleonThrp_staple(*seqPropOut1, *propF, *WL1, signProps1, gammas, t, z, 0);
        if(signPer < 0) for(int iv = 0 ; iv < corrThrpWL.getTotalSize()*2; iv++) corrThrpWL.H_elem()[iv] *= signPer;
        corrThrpWL.writeHDF5( (threep_filename +".CP1_ts_"+std::to_string(tSinks[ts])+"_stout_"+std::to_string(nStout)+".h5").c_str());
        // CP2
        // get fwd prop based on the nucleon
        ...
        // contract the three point function
        corrThrpWL.contractNucleonThrp_staple(*seqPropOut2, *propF, *WL1, signProps2, gammas, t, z, 0);
        if(signPer < 0) for(int iv = 0 ; iv < corrThrpWL.getTotalSize()*2; iv++) corrThrpWL.H_elem()[iv] *= signPer;
        corrThrpWL.writeHDF5( (threep_filename +".CP2_ts_"+std::to_string(tSinks[ts])+"_stout_"+std::to_string(nStout)+".h5").c_str());
      } // loop over Z
      // negative Z
      // get the field along Z direction
      su3_2->absorbDir_device(gaugeWL, Zdir);
