...
*/
  ...
  //for(size_t its = 0; its < tSinks.size(); its++){
    //int tsink = tSinks[its];
      ...
      std::vector<std::shared_ptr<PLEGMA_Vector<double>>> vecs;
    	for(int i=0;i<np;i++) vecs.push_back(std::make_shared<PLEGMA_Vector<double>>(DEVICE));
      ...
      PLEGMA_ScattCorrelator<double> AccCorr(site({0,0,0,tsink}), maxQsq, eigvecnum);
      AccCorr.initialize_diagram(glist_src, glist_sink, "P");
      ...
      for(int i=0; i < eigSol->getEigVals().size(); i++){
        ...
        for(int j=i; j < eigSol->getEigVals().size(); j+=np){
          TIME(
          int maxnp = std::min(np,(int)eigSol->getEigVals().size()-j);
          for(int k=0;k<maxnp;k++) {
            iorder = std::get<3>(eigSol->getEigVals()[j+k]);
            eigVec_tmp = eigSol->getEigVecs() + iorder*eigSol->getSize_per_Vec()*2;
            qudaMemcpyAsync(vecs[k]->D_elem(), eigVec_tmp, eigSol->getBytes_per_Vec(), qudaMemcpyHostToDevice, device::get_stream(k));
          }

          for(int k=0;k<maxnp;k++) {
            qudaStreamSynchronize(device::get_stream(k));
            corr.PhiPhi(eigVec, glist_src, *vecs[k]);
            AccCorr.absorbEigIndex(corr, eigindex);
            eigindex++;
          }
          );
        }
    ...
    TIME(AccCorr.writeHDF5( outfilename ));
      /*
    //=========================================================================================================//
    ...
    //{
      ...
      for(int imu=0; imu<nmus; imu++){ //loop over all mus.
        ...
        for(int fl=0; fl<2; fl++){
          ...
          TIME(stoc_corr.contractMesonsNew(propUP, propUP));
