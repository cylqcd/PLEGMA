...
int main(int argc, char **argv) {
...

  {
    ...
    PLEGMA_Gauge<double> contractGauge(BOTH, THIRD_SIDE);
    {
	...
	PLEGMA_Propagator<double> propUP_SL(tSinks.size()>0 ? BOTH:NONE, THIRD_SIDE);
	PLEGMA_Propagator<double> propDN_SL(tSinks.size()>0 ? BOTH:NONE, THIRD_SIDE);
	...
	for(size_t its = 0; its < tSinks.size(); its++){
	  ...
	  for(size_t iproj = 0; iproj < Projs.size(); iproj++){
		...
		computed_light = true;
	      ...
	      PLEGMA_Propagator<double> seqProp(BOTH, THIRD_SIDE);
		...
		for(int c2 = 0 ; c2 < 3 ; c2++){
	      ...
	      PLEGMA_Correlator<double> corr3(corr_space, source, 0, tsinkMtsource+1);
