...
int main(int argc, char **argv) {
...

  {
    ...
    //   updateOptions(srcInputFile + std::to_string(isource), listOpt, add_options);
      ...
      auto computePropagator = [&](PLEGMA_Propagator<float>& prop_SS, PLEGMA_Propagator<float>& prop_SL,
				...
				//  }
				 for(int isc = 0 ; isc < 12 ; isc++){
				   ...
				   TIME(eigSol->solve(vectorInOut, run_mu));
      ...
      char * src_string;
      asprintf(&src_string, "_sx%02dsy%02dsz%02dst%03d_nev%03d", source[0], source[1], source[2], source[3], Eig_NeV);
...
// #ifdef PLEGMA_NUCLEON_3PF_FIX_SINK
	for(size_t its = 0; its < tSinks.size(); its++){
	  ...
	  for(size_t iproj = 0; iproj < Projs.size(); iproj++){
		...
		if(nu==0 && c2==0){
			vectorAuxF.unload();
			vectorAuxF.writeHDF5("/leonardo_scratch/large/userexternal/cschneid/B64/nucl_defl_3pt_cc/vecAuxF.h5");
		}
