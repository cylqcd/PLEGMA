...
//   std::string light_prop_filename;
  ...
  auto add_options = [&](Options& options) {
    options.set("kappa-c", "kappa_c to run for the strange quark in tetraquarks", verbosity, kappa_c);
//     options.set("mu-c", "List of mu_c to run for the charm quark in baryons", verbosity, mu_c);
//     options.set("nsmear-gauss-s", "Number of Gaussian smearing step for the strange quark propagator", verbosity, nsmearGauss_c);
...
//              prop.applyBoundaries_device(source[DIM_T]);
       ...
       TIME(computePropagator(propCH, kappa_c, CHARM, nsmearGauss_c));
        ...
        PLEGMA_Correlator<double> corr(corr_space, source, 2);
        char * group;
        
        
        TIME(corr.contractTetraquarksBCUD(propLT, none, propCH, propBT));
        ...
        PLEGMA_Correlator<double> corr_scattering(corr_space, source, maxQsq);
        ...
        for (int i=0; i<gammas.size(); i++){
            ...
            for (int s1=0; s1< N_SPINS; s1++){ // this sum is required as the GPU memory is not large enough to cover all 4*4*3*3 elements in double precision
                TIME(corr_scattering.contractTetraquarkScatteringOpenIndex(propBT, propLT, gamma, s1,"BU"));
                THREAD(corr_scattering.writeFile(openContr_filename, corr_file_format));
                
                TIME(corr_scattering.contractTetraquarkScatteringOpenIndex(propCH, propLT, gamma, s1,"DU"));
                THREAD(corr_scattering.writeFile(openContr_filename, corr_file_format));
            }
        }
...
//         THREAD(corr.writeFile(twop_filename, corr_file_format));
        ...
        TIME(corr_meson.contractMesonsNew(propLT, propCH ));
        asprintf(&group, "D_mesons");
        corr_meson.setGroups(group);
        free(group);
        THREAD(corr_meson.writeFile(twop_filename, corr_file_format));
        ...
        TIME(corr_meson.contractMesonsNew(propCH, propCH ));
        asprintf(&group, "Charm_mesons");
        corr_meson.setGroups(group);
        free(group);
        THREAD(corr_meson.writeFile(twop_filename, corr_file_format));
        
        
        TIME(corr_meson.contractMesonsNew(propBT, propCH ));
        asprintf(&group, "Bd_mesons");
        corr_meson.setGroups(group);
        free(group);
        THREAD(corr_meson.writeFile(twop_filename, corr_file_format));
