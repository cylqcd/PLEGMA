...
#include <PLEGMA_utils.h>
  ...
  int main(int argc, char **argv)
  {
	...
	if(outfile_S!="")
	  {
	    ...
	    for(int isc = 0 ; isc < 12 ; isc++){
	      ...
	      vectorAuxPrint.writeHDF5((outfile_S+"_s"+spin+"_c"+col));
	    }
	  }
      }
      else {
	...
	if(outfile_V!=""){
	  ...
	  vectorStoc.unload();
	  vectorStoc.writeHDF5((outfile_V+"1"));

	  PLEGMA_printf("Print Vector2\n");
	  vectorStoc2.unload();
	  vectorStoc2.writeHDF5((outfile_V+"2"));
 ...
 /*   {
	...
	reductions.writeHDF5(outfile_V5+"conj_true");
    }
    {
      PLEGMA_ScattCorrelator<float> reductions(sourcePositions[0], sourcemomentumList.uniq_p(1));
      reductions.V5( vectorStoc, vectorStoc2,false);
      double norm=vectorStoc.norm();
      PLEGMA_printf("Norm of vector after V5 reduction %d\n",norm);
    ...
    }
    {
      ...
      PLEGMA_ScattCorrelator<float> reductions(sourcePositions[0], sourcemomentumList.uniq_p(1));
      reductions.V3( vectorStoc,glist1, propUP,false);
      double norm=vectorStoc.norm();
      PLEGMA_printf("Norm of vector after V3 reduction %e\n",norm);
      reductions.writeHDF5(outfile_V3+"_1mom_c1");
    }

    {
      PLEGMA_ScattCorrelator<float> reductions(sourcePositions[0], sourcemomentumList.uniq_p(1));
      reductions.V3( vectorStoc,glist2, propUP,false);
      double norm=vectorStoc.norm();
      PLEGMA_printf("Norm of vector after V3 reduction %e\n",norm);
	...
	reductions.writeHDF5(outfile_V6+"conj_true");
    }
    {
      PLEGMA_ScattCorrelator<float> reductions(sourcePositions[0], sourcemomentumList.uniq_p(1));
      reductions.V6( vectorStoc,vectorStoc2,propUP,false);
      double norm=vectorStoc.norm();
      PLEGMA_printf("Norm of vector after V6 reduction\n",norm);
