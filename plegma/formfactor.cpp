...
int main(int argc, char **argv)
{
  ...
  PLEGMA_Propagator<float> *propUPm[numSourcePositions];
  PLEGMA_Propagator<float> *propDNp[numSourcePositions];
  PLEGMA_Propagator<float> *propDNm[numSourcePositions];
  
  for(int is = 0; is < numSourcePositions; is++) {
    propUPp[is] = new PLEGMA_Propagator<float>(BOTH);
    propUPm[is] = new PLEGMA_Propagator<float>(BOTH);
    propDNp[is] = new PLEGMA_Propagator<float>(BOTH);
    propDNm[is] = new PLEGMA_Propagator<float>(BOTH);
  }
  ...
  for(int isource = 0; isource < numSourcePositions; isource++) { //loop over source positions
    ...
    for(int isc=0;isc<12;isc++){
      ...
      solver->solve(vectorOut,vectorIn);
      vectorAuxF.copy(vectorOut);
      propUPm[isource]->absorb(vectorAuxF, isc/3, isc%3);
    }
  ...
  for(int isource = 0; isource < numSourcePositions; isource++) { //loop over source positions
    ...
    for(int isc=0;isc<12;isc++){
      ...
      solver->solve(vectorOut,vectorIn);
      vectorAuxF.copy(vectorOut);
      propDNm[isource]->absorb(vectorAuxF, isc/3, isc%3);
    }
    propDNp[isource]->rotateToPhysicalBase_device(-1);
    propDNp[isource]->applyBoundaries_device(0);
    propDNm[isource]->rotateToPhysicalBase_device(-1);
    propDNm[isource]->applyBoundaries_device(0);
  ...
  for(int is = 0; is < numSourcePositions; is++) { //loop over source positions
    ...
    for(int b = 0; b < B_max; b++) { //loop over values of B
      ...
      prop1->copy(*propUPp[is]);
      prop2->copy(*propUPm[is]);
      ...
      corr4p.contractMesonsFourp_ultralocal(*prop2, *prop1, *propDNp[is], *propDNm[is], +b);
