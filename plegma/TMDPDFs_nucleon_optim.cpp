...
int main(int argc, char **argv)
{
  ...
  size_t B_max=1;
  HGC_options->set("Bmax", "Maximum number of transverse separations", verbosity, B_max);
  ...
  for(int l:Lvals) {//loop over values of L
    ...
    for(int li = 0; li < l; li++) {
      WL_1->wilsonLineUpdate(*su3_4_1, *su3_exchange, 4 + WilsDir);
    }
    WL_4_1->copy(*WL_1);
    WL_4_2->copy(*WL_1);
    WL_4_2->Udag();
    ...
    for(int z = 0; z < l; z++) {//loop  over values of Z
      if(z!=0){
        WL_4_1->wilsonLineUpdate(*su3_4_1, *su3_exchange, 4 + WilsDir, true);
        WL_4_2->wilsonLineUpdate(*su3_4_2, *su3_exchange, WilsDir);
      }
      //positive b
      // positive z
      for(int zi = 0; zi < z; zi++) {
        propExchange = propIn; propIn = propUP; propUP = propExchange;
        propUP->shift(*propIn, WilsDir);
        propExchange = propIn; propIn = propDN; propDN = propExchange;
        propDN->shift(*propIn, WilsDir);
        propExchange = propIn; propIn = seqPropOut1; seqPropOut1 = propExchange;
        seqPropOut1->shift(*propIn, 4 + WilsDir);
        propExchange = propIn; propIn = seqPropOut2; seqPropOut2 = propExchange;
        seqPropOut2->shift(*propIn, 4 + WilsDir);
      }
      // Bdir1
      WL_2->copy(*WL_4_1);
      su3_2->absorbDir_device(gaugeWL, Bdir1);
      for(int li = 0; li < l; li++) {
        su3_exchange = su3_in; su3_in = su3_2; su3_2 = su3_exchange;
        su3_2->shift(*su3_in, 4 + WilsDir);
      }
      for(int b = 0; b <= B_max; b++) {//loop over values of B
        if(b != 0) {
          WL_2->wilsonLineUpdate(*su3_2, *su3_exchange, 4 + Bdir1);
          su3_exchange = su3_in; su3_in = WL_4_2; WL_4_2 = su3_exchange;
          WL_4_2->shift(*su3_in, 4 + Bdir1);
          propExchange = propIn; propIn = propUP; propUP = propExchange;
          propUP->shift(*propIn, 4 + Bdir1);
          propExchange = propIn; propIn = propDN; propDN = propExchange;
          propDN->shift(*propIn, 4 + Bdir1);
        }
        WL1->UxU(*WL_2,*WL_4_2);

        if(nucleon == PROTON) corrThrpWL.contractNucleonThrp_staple(*seqPropOut1, *propDN, *WL1, signProps1, gammas, l, b, z);
        else corrThrpWL.contractNucleonThrp_staple(*seqPropOut1, *propUP, *WL1, signProps1, gammas, l, b, z);
        ...
        if(nucleon == PROTON) corrThrpWL.contractNucleonThrp_staple(*seqPropOut2, *propUP, *WL1, signProps1, gammas, l, b, z);
        else corrThrpWL.contractNucleonThrp_staple(*seqPropOut2, *propDN, *WL1, signProps1, gammas, l, b, z);
        if(signPer < 0) for(int iv = 0 ; iv < corrThrpWL.getTotalSize()*2; iv++) corrThrpWL.H_elem()[iv] *= signPer;
      ...
      for(int bi = 0; bi < B_max; bi++) {
        ...
        propExchange = propIn; propIn = propUP; propUP = propExchange;
        propUP->shift(*propIn, Bdir1);
        propExchange = propIn; propIn = propDN; propDN = propExchange;
        propDN->shift(*propIn, Bdir1);
      }
      // Bdir2
      WL_2->copy(*WL_4_1);
      su3_2->absorbDir_device(gaugeWL, Bdir2);
      for(int li = 0; li < l; li++) {
        su3_exchange = su3_in; su3_in = su3_2; su3_2 = su3_exchange;
        su3_2->shift(*su3_in, 4 + WilsDir);
      }
      for(int b = 0; b <= B_max; b++) {//loop over values of B
        if(b != 0) {
          WL_2->wilsonLineUpdate(*su3_2, *su3_exchange, 4 + Bdir2);
          su3_exchange = su3_in; su3_in = WL_4_2; WL_4_2 = su3_exchange;
          WL_4_2->shift(*su3_in, 4 + Bdir2);
          propExchange = propIn; propIn = propUP; propUP = propExchange;
          propUP->shift(*propIn, 4 + Bdir2);
          propExchange = propIn; propIn = propDN; propDN = propExchange;
          propDN->shift(*propIn, 4 + Bdir2);
        }
        WL1->UxU(*WL_2,*WL_4_2);
      ...
      for(int bi = 0; bi < B_max; bi++) {
        ...
        propExchange = propIn; propIn = propUP; propUP = propExchange;
        propUP->shift(*propIn, Bdir2);
        propExchange = propIn; propIn = propDN; propDN = propExchange;
        propDN->shift(*propIn, Bdir2);
      }
      // negative z
      for(int zi = 0; zi < 2*z; zi++) {
        propExchange = propIn; propIn = propUP; propUP = propExchange;
        propUP->shift(*propIn, 4 + WilsDir);
        propExchange = propIn; propIn = propDN; propDN = propExchange;
        propDN->shift(*propIn, 4 + WilsDir);
        propExchange = propIn; propIn = seqPropOut1; seqPropOut1 = propExchange;
        seqPropOut1->shift(*propIn, WilsDir);
        propExchange = propIn; propIn = seqPropOut2; seqPropOut2 = propExchange;
        seqPropOut2->shift(*propIn, WilsDir);
      }
      // Bdir1
      WL_2->copy(*WL_4_2);
      WL_2->Udag();
      ...
      for(int b = 0; b <= B_max; b++) {//loop over values of B
        if(b != 0) {
          WL_2->wilsonLineUpdate(*su3_2, *su3_exchange, Bdir1);
          su3_exchange = su3_in; su3_in = WL_4_2; WL_4_2 = su3_exchange;
          WL_4_2->shift(*su3_in, Bdir1);
          propExchange = propIn; propIn = propUP; propUP = propExchange;
          propUP->shift(*propIn, Bdir1);
      ...
      for(int b = 0; b <= B_max; b++) {//loop over values of B
        if(b != 0) {
          WL_2->wilsonLineUpdate(*su3_2, *su3_exchange, Bdir2);
          su3_exchange = su3_in; su3_in = WL_4_2; WL_4_2 = su3_exchange;
          WL_4_2->shift(*su3_in, Bdir2);
          propExchange = propIn; propIn = propUP; propUP = propExchange;
          propUP->shift(*propIn, Bdir2);
  ...
  for(int l:Lvals) {//loop over values of L
    ...
    WL_1->setUnit( (std::vector<int>) {0,4,8});
    for(int li = 0; li < l; li++) {
      WL_1->wilsonLineUpdate(*su3_4_1, *su3_exchange, WilsDir);
    }
    WL_4_1->copy(*WL_1);
    WL_4_2->copy(*WL_1);
    WL_4_2->Udag();
    ...
    for(int z = 0; z < l; z++) {//loop  over values of Z
      if(z!=0){
        WL_4_1->wilsonLineUpdate(*su3_4_1, *su3_exchange, WilsDir, true);
        WL_4_2->wilsonLineUpdate(*su3_4_2, *su3_exchange, 4 + WilsDir);
      }
      //positive b
      // positive z
      for(int zi = 0; zi < z; zi++) {
  ...
  //for(int is = 0; is < numSourcePositions; is++) {//loop over source positions
    ...
    corr.contractBaryons(*propUP_SS, *propDN_SS);
