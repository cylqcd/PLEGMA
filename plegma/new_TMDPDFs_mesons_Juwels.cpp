...
int main(int argc, char **argv)
{
  ...
  std::vector<PLEGMA_Propagator<float>*> sequential_propagators;

  for(int i=0; i< 6*tSinks.size(); ++i) {
    sequential_propagators.push_back(new PLEGMA_Propagator<float>(HOST));
  }
  ...
  for(int ts=0;ts<tSinks.size();ts++)
	...
	sequential_propagators[(6*ts)+0]->copy(*seqProp_pionUPplusmom,HOST);
	sequential_propagators[(6*ts)+1]->copy(*seqProp_pionUPminusmom,HOST);
	sequential_propagators[(6*ts)+2]->copy(*seqProp_kaonUPplusmom,HOST);
        sequential_propagators[(6*ts)+3]->copy(*seqProp_kaonUPminusmom,HOST);
	sequential_propagators[(6*ts)+4]->copy(*seqProp_kaonSTplusmom,HOST);
        sequential_propagators[(6*ts)+5]->copy(*seqProp_kaonSTminusmom,HOST);
	
      }
    }

  PLEGMA_printf("DONE PROPAGATORS \n");
  ...
  for(int l = 0; l <= L_LEN; l++) {//loop over values of L, building "1" and "3"
	...
	for(int ts=0;ts<tSinks.size();ts++){
	  ...
	  PLEGMA_Propagator<float> sequential_propagator_0;
          PLEGMA_Propagator<float> sequential_propagator_1;
	  PLEGMA_Propagator<float> sequential_propagator_2;
          PLEGMA_Propagator<float> sequential_propagator_3;
	  PLEGMA_Propagator<float> sequential_propagator_4;
          PLEGMA_Propagator<float> sequential_propagator_5;
            
          sequential_propagator_0.copy(*sequential_propagators[0+(ts*6)],HOST);
	  sequential_propagator_1.copy(*sequential_propagators[1+(ts*6)],HOST);
	  sequential_propagator_2.copy(*sequential_propagators[2+(ts*6)],HOST);
          sequential_propagator_3.copy(*sequential_propagators[3+(ts*6)],HOST);
	  sequential_propagator_4.copy(*sequential_propagators[4+(ts*6)],HOST);
          sequential_propagator_5.copy(*sequential_propagators[5+(ts*6)],HOST);

	  sequential_propagator_0.load();
          sequential_propagator_1.load();
          sequential_propagator_2.load();
          sequential_propagator_3.load();
          sequential_propagator_4.load();
          sequential_propagator_5.load();
	  
	  PLEGMA_printf("GET HERE \n");
	  
	  TIME(corrThrpWL.contractNucleonThrp_staple(sequential_propagator_0, *propF_UPsmearplus_1, *WL1, +1, gammas, false, bval, -l, -zval));
	  corrThrpWL.writeFile((threep_filename+"_ts"+std::to_string(tSinks[ts])+"_pionUP_plusmom_B_DIR1_"+conf).c_str(), corr_file_format);

	  TIME(corrThrpWL.contractNucleonThrp_staple(sequential_propagator_0, *propF_UPsmearplus_2, *WL2, +1, gammas, false, bval, -l, -zval));
	  corrThrpWL.writeFile((threep_filename+"_ts"+std::to_string(tSinks[ts])+"_pionUP_plusmom_B_DIR2_"+conf).c_str(), corr_file_format);

	  TIME(corrThrpWL.contractNucleonThrp_staple(sequential_propagator_1, *propF_UPsmearminus_1, *WL1, +1, gammas, false, bval, -l, -zval));
          corrThrpWL.writeFile((threep_filename+"_ts"+std::to_string(tSinks[ts])+"_pionUP_minusmom_B_DIR1_"+conf).c_str(), corr_file_format);

          TIME(corrThrpWL.contractNucleonThrp_staple(sequential_propagator_1, *propF_UPsmearminus_2, *WL2, +1, gammas, false, bval, -l, -zval));
          corrThrpWL.writeFile((threep_filename+"_ts"+std::to_string(tSinks[ts])+"_pionUP_minusmom_B_DIR2_"+conf).c_str(), corr_file_format);

	  TIME(corrThrpWL.contractNucleonThrp_staple(sequential_propagator_2, *propF_UPsmearplus_1, *WL1, +1, gammas, false, bval, -l, -zval));
	  corrThrpWL.writeFile((threep_filename_1+"_ts"+std::to_string(tSinks[ts])+"_kaonUP_plusmom_B_DIR1_"+conf).c_str(), corr_file_format);
	      
	  TIME(corrThrpWL.contractNucleonThrp_staple(sequential_propagator_2, *propF_UPsmearplus_2, *WL2, +1, gammas, false, bval, -l, -zval));
	  corrThrpWL.writeFile((threep_filename_1+"_ts"+std::to_string(tSinks[ts])+"_kaonUP_plusmom_B_DIR2_"+conf).c_str(), corr_file_format);

	  TIME(corrThrpWL.contractNucleonThrp_staple(sequential_propagator_3, *propF_UPsmearminus_1, *WL1, +1, gammas, false, bval, -l, -zval));
          corrThrpWL.writeFile((threep_filename_1+"_ts"+std::to_string(tSinks[ts])+"_kaonUP_minusmom_B_DIR1_"+conf).c_str(), corr_file_format);

          TIME(corrThrpWL.contractNucleonThrp_staple(sequential_propagator_3, *propF_UPsmearminus_2, *WL2, +1, gammas, false, bval, -l, -zval));
          corrThrpWL.writeFile((threep_filename_1+"_ts"+std::to_string(tSinks[ts])+"_kaonUP_minusmom_B_DIR2_"+conf).c_str(), corr_file_format);

	  TIME(corrThrpWL.contractNucleonThrp_staple(sequential_propagator_4, *propF_STsmearplus_1, *WL1, +1, gammas, false, bval, -l, -zval));
          corrThrpWL.writeFile((threep_filename_1+"_ts"+std::to_string(tSinks[ts])+"_kaonST_plusmom_B_DIR1_"+conf).c_str(), corr_file_format);

          TIME(corrThrpWL.contractNucleonThrp_staple(sequential_propagator_4, *propF_STsmearplus_2, *WL2, +1, gammas, false, bval, -l, -zval));
          corrThrpWL.writeFile((threep_filename_1+"_ts"+std::to_string(tSinks[ts])+"_kaonST_plusmom_B_DIR2_"+conf).c_str(), corr_file_format);

          TIME(corrThrpWL.contractNucleonThrp_staple(sequential_propagator_5, *propF_STsmearminus_1, *WL1, +1, gammas, false, bval, -l, -zval));
          corrThrpWL.writeFile((threep_filename_1+"_ts"+std::to_string(tSinks[ts])+"_kaonST_minusmom_B_DIR1_"+conf).c_str(), corr_file_format);

          TIME(corrThrpWL.contractNucleonThrp_staple(sequential_propagator_5, *propF_STsmearminus_2, *WL2, +1, gammas, false, bval, -l, -zval));
  ...
  for(int l = 0; l <= L_LEN; l++) {//loop over values of L, building "1" and "3"
	...
	for(int ts=0;ts<tSinks.size();ts++){
	  ...
	  PLEGMA_Correlator<float> corrThrpWL_1(corr_space, sourcePositions[isource], 0, tSinks[ts]+1);

	  PLEGMA_Propagator<float> sequential_propagator_0_1;
          PLEGMA_Propagator<float> sequential_propagator_1_1;
          PLEGMA_Propagator<float> sequential_propagator_2_1;
          PLEGMA_Propagator<float> sequential_propagator_3_1;
          PLEGMA_Propagator<float> sequential_propagator_4_1;
          PLEGMA_Propagator<float> sequential_propagator_5_1;

          sequential_propagator_0_1.copy(*sequential_propagators[0+(ts*6)],HOST);
          sequential_propagator_1_1.copy(*sequential_propagators[1+(ts*6)],HOST);
          sequential_propagator_2_1.copy(*sequential_propagators[2+(ts*6)],HOST);
          sequential_propagator_3_1.copy(*sequential_propagators[3+(ts*6)],HOST);
          sequential_propagator_4_1.copy(*sequential_propagators[4+(ts*6)],HOST);
          sequential_propagator_5_1.copy(*sequential_propagators[5+(ts*6)],HOST);

	  sequential_propagator_0_1.load();
          sequential_propagator_1_1.load();
          sequential_propagator_2_1.load();
          sequential_propagator_3_1.load();
          sequential_propagator_4_1.load();
          sequential_propagator_5_1.load();
	  
	  TIME(corrThrpWL_1.contractNucleonThrp_staple(sequential_propagator_0_1, *propF_UPsmearplus_1, *WL1, +1, gammas, false, bval, l, zval));
          corrThrpWL_1.writeFile((threep_filename+"_ts"+std::to_string(tSinks[ts])+"_pionUP_plusmom_B_DIR1_"+conf).c_str(), corr_file_format);

	  TIME(corrThrpWL_1.contractNucleonThrp_staple(sequential_propagator_0_1, *propF_UPsmearplus_2, *WL2, +1, gammas, false, bval, l, zval));
          corrThrpWL_1.writeFile((threep_filename+"_ts"+std::to_string(tSinks[ts])+"_pionUP_plusmom_B_DIR2_"+conf).c_str(), corr_file_format);

          TIME(corrThrpWL_1.contractNucleonThrp_staple(sequential_propagator_1_1, *propF_UPsmearminus_1, *WL1, +1, gammas, false, bval, l, zval));
          corrThrpWL_1.writeFile((threep_filename+"_ts"+std::to_string(tSinks[ts])+"_pionUP_minusmom_B_DIR1_"+conf).c_str(), corr_file_format);

          TIME(corrThrpWL_1.contractNucleonThrp_staple(sequential_propagator_1_1, *propF_UPsmearminus_2, *WL2, +1, gammas, false, bval, l, zval));
          corrThrpWL_1.writeFile((threep_filename+"_ts"+std::to_string(tSinks[ts])+"_pionUP_minusmom_B_DIR2_"+conf).c_str(), corr_file_format);

          TIME(corrThrpWL_1.contractNucleonThrp_staple(sequential_propagator_2_1, *propF_UPsmearplus_1, *WL1, +1, gammas, false, bval, l, zval));
          corrThrpWL_1.writeFile((threep_filename_1+"_ts"+std::to_string(tSinks[ts])+"_kaonUP_plusmom_B_DIR1_"+conf).c_str(), corr_file_format);

          TIME(corrThrpWL_1.contractNucleonThrp_staple(sequential_propagator_2_1, *propF_UPsmearplus_2, *WL2, +1, gammas, false, bval, l, zval));
          corrThrpWL_1.writeFile((threep_filename_1+"_ts"+std::to_string(tSinks[ts])+"_kaonUP_plusmom_B_DIR2_"+conf).c_str(), corr_file_format);

          TIME(corrThrpWL_1.contractNucleonThrp_staple(sequential_propagator_3_1, *propF_UPsmearminus_1, *WL1, +1, gammas, false, bval, l, zval));
          corrThrpWL_1.writeFile((threep_filename_1+"_ts"+std::to_string(tSinks[ts])+"_kaonUP_minusmom_B_DIR1_"+conf).c_str(), corr_file_format);

          TIME(corrThrpWL_1.contractNucleonThrp_staple(sequential_propagator_3_1, *propF_UPsmearminus_2, *WL2, +1, gammas, false, bval, l, zval));
          corrThrpWL_1.writeFile((threep_filename_1+"_ts"+std::to_string(tSinks[ts])+"_kaonUP_minusmom_B_DIR2_"+conf).c_str(), corr_file_format);

          TIME(corrThrpWL_1.contractNucleonThrp_staple(sequential_propagator_4_1, *propF_STsmearplus_1, *WL1, +1, gammas, false, bval, l, zval));
          corrThrpWL_1.writeFile((threep_filename_1+"_ts"+std::to_string(tSinks[ts])+"_kaonST_plusmom_B_DIR1_"+conf).c_str(), corr_file_format);

          TIME(corrThrpWL_1.contractNucleonThrp_staple(sequential_propagator_4_1, *propF_STsmearplus_2, *WL2, +1, gammas, false, bval, l, zval));
          corrThrpWL_1.writeFile((threep_filename_1+"_ts"+std::to_string(tSinks[ts])+"_kaonST_plusmom_B_DIR2_"+conf).c_str(), corr_file_format);

          TIME(corrThrpWL_1.contractNucleonThrp_staple(sequential_propagator_5_1, *propF_STsmearminus_1, *WL1, +1, gammas, false, bval, l, zval));
          corrThrpWL_1.writeFile((threep_filename_1+"_ts"+std::to_string(tSinks[ts])+"_kaonST_minusmom_B_DIR1_"+conf).c_str(), corr_file_format);

          TIME(corrThrpWL_1.contractNucleonThrp_staple(sequential_propagator_5_1, *propF_STsmearminus_2, *WL2, +1, gammas, false, bval, l, zval));
          corrThrpWL_1.writeFile((threep_filename_1+"_ts"+std::to_string(tSinks[ts])+"_kaonST_minusmom_B_DIR2_"+conf).c_str(), corr_file_format);
	}
      }//loop over values of Z, building "4"
    }//loop over values of B, building "2"			
  }//loop over values of L, building "1" and "3"
  ...
  for(int i=0; i< 6*tSinks.size(); ++i) {
    sequential_propagators.pop_back();
  }
