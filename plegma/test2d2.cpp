...
int main(int argc, char **argv)
{
  ...
  // Allocation done on BOTH, DEVICE and HOST
  {
    ...
    PLEGMA_Gauge<float> gauge2(BOTH,THIRD_SIDE);
    ...
    PLEGMA_Propagator<float> prop2(BOTH,THIRD_SIDE);
    ...
    prop.readFile("/eagle/NucleonForm/ckummer/test2d/prop64.lime", LIME_FORMAT);

    prop2.copy(prop);

    //prop.load();
    double norm=prop2.norm();
    ...
    corr.contractNucleonThrp_local(prop2,prop2,1,gammas,false);
    corr.writeFile(twop_filename,corr_file_format);

    corr.contractNucleonThrp_oneD(prop2,prop2,gauge2,1,gammas,false);
    corr.writeFile(twop_filename,corr_file_format);

    corr.contractNucleonThrp_twoD(prop2,prop2,gauge2,1,gammas,false);
    corr.writeFile(twop_filename,corr_file_format);

    corr.contractNucleonThrp_threeD(prop2,prop2,gauge2,1,gammas,false);
    corr.writeFile(twop_filename,corr_file_format);
