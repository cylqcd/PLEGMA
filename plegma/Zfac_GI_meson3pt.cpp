...
int main(int argc, char **argv) {
...

  {
	...
	//s//PLEGMA_Propagator<float> propUP_SL(tSinks.size()>0 ? BOTH:NONE, FIRST_CORNER);
	//s//PLEGMA_Propagator<float> propST_SL(tSinks.size()>0 ? BOTH:NONE, FIRST_CORNER);
	...
	// Correcting the smearing for strange
	/*
	for(int isc = 0 ; isc < 12 ; isc++){
	  PLEGMA_Vector<double> vectorAuxD, vectorAuxD2;
	  PLEGMA_Vector<float> vectorAuxF;
	...
	for(size_t its = 0; its < tSinks.size(); its++){
	  ...
	  for(size_t imom = 0; imom < sourceMom.size()/3; imom++){
	    ...
	    auto computeThreep = [&](double run_mu, PLEGMA_Propagator3D<float>& prop, PLEGMA_Propagator<float> &propF, int nSmear, WHICHFLAVOR fl, std::string name) {
	      ...
	      std::string filename0 = threep_filename + mom_string + "_dt" + std::to_string(tsinkMtsource)+name;
	      free(mom_string);
