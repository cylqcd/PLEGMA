...
int main(int argc, char **argv) {
  ...
  HGC.options->set("confnumber", "Integer determining the index of the gauge configuration", verbosity, confnumber_int);
  HGC.options->set("maxSourceSinkSeparations", "Maximal source sink separations", verbosity, max_source_sink_separations);
  HGC.options->set("outdiagramPrefix", "Prefix of the resulting diagrams", verbosity, outdiagramPrefix);
  HGC.options->set("nstochSamples", "Number of stochastic samples", verbosity, n_stochastic_samples);
  HGC.options->set("seed1", "Seed for initialization of stochastic sources for the oet", verbosity, rand_seed1);
  HGC.options->set("seed2", "Seed for intiialization of stochastic sources", verbosity, rand_seed2);
  HGC.options->set("dotwopoint", "Doing also the twopoint functions", verbosity,dotwopoint);
  HGC.options->set("readStochSamples", "Flag for switching read/building stochastic propagators", verbosity, readStochSamples);
  ...
  if(comm_rank() == 0){
    ...
    for(int i = 0 ; i < N_DIMS; i++) oss << "<l" << xyzt[i] << ">" << HGC.totalL[i] << "</l" << xyzt[i] << ">\n";
    oss << "</ildgFormat>";
...
//  write_binary_to_lime(filename,fid,limewriter,h_elem,field_length);
...

  {
    ...
    int parallel_sources=HGC.totalL[3]/max_source_sink_separations;
    std::vector<int> lookuptable_UP;
    std::vector<int> lookuptable_DN;

    
    for (int i=0; i<HGC.totalL[3]; ++i){
      lookuptable_UP.push_back(-1);
      lookuptable_DN.push_back(-1);
    }

    int *attract_lookup_table;
    attract_lookup_table=(int *)malloc(sizeof(int)*HGC.totalL[3]);
	...
	//vectorRead.readFile("stochastic_source0.lime",LIME_FORMAT);
	//vectorRead.load();
    ...
    if ((dotwopoint==1) && (readStochSamples==0)){
      for (int i=0; i<n_stochastic_samples;++i){
        ...
        for (int timeidx=0; timeidx< HGC.totalL[DIM_T]; ++timeidx){
    ...
    for (int isource=0; isource<numSourcePositions;++isource){
      ...
      for (int k=0; k<tSinks.size();++k){
        for (int l=0; l<parallel_sources;++l){
          if (lookuptable_UP[(source[3]+tSinks[k]+l*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3]]==-1){
            int timeSlice=(source[3]+tSinks[k]+l*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3];	     
    ...
    for (int isource=0; isource<numSourcePositions;++isource){
      ...
      for (int k=0; k<tSinks.size();++k){
        for (int l=0; l<parallel_sources;++l){
          if (lookuptable_DN[(source[3]+tSinks[k]+l*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3]]==-1){
            int timeSlice=(source[3]+tSinks[k]+l*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3];
            PLEGMA_Gauge3D<double> smearedGauge3D;
            smearedGauge3D.absorb(smearedGauge, timeSlice );
            for(int i=0; i< n_stochastic_samples; ++i) {
		...
		vectorInOut.copy(vectorSource_stochastic);
	      ...
	      stochastic_propags_DN_SL[countindex]->copy(vectorAuxD1, HOST);
            ...
            //lookuptable[(source[3]+tSinks[k]+l*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3]]=isource*tSinks.size()*parallel_sources+k*parallel_sources+l;
    ...
    for(int isource = startSource; isource < numSourcePositions; isource++){
      ...
      auto computePropagator = [&](PLEGMA_Propagator<float>& prop_SS, PLEGMA_Propagator<float>& prop_SL,
				 ...
				 for(int isc = 0 ; isc < 12 ; isc++){
				   ...
				   //vectorInOut.unload();
				   //vectorInOut.writeHDF5("testPointSourceSmeared");
				   //vectorInOut.load();
				   // Inverting
	...
	for (int sink=0; sink<max_source_sink_separations;++sink){
	  attract_lookup_table[(sourcePositions[isource][DIM_T]+max_source_sink_separations*i_source_parallel+sink)%HGC.totalL[3]]=(sourcePositions[isource][DIM_T]+i_source_parallel*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3];
	}
      }
      for (int i_source_parallel=0; i_source_parallel < parallel_sources;++i_source_parallel){
	...
	//source_local[3]=(sourcePositions[isource][3]+i_source_parallel*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3];
        site source_local;
	source_local[0] = sourcePositions[isource][0];
	source_local[1] = sourcePositions[isource][1];
	source_local[2] = sourcePositions[isource][2];
	source_local[3] = (sourcePositions[isource][DIM_T]+i_source_parallel*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3];
	...
	source_localPtSinkMtSource[0] = sourcePositions[isource][0];
        ...
        source_localPtSinkMtSource[3] = (sourcePositions[isource][DIM_T]+(i_source_parallel+1)*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3];

	site source_local_reduction=site({0,0,0,sourcePositions[isource][DIM_T]});
        source_local_reduction[3]=(sourcePositions[isource][3]+i_source_parallel*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3];
...
//        TIME(corrN0.initialize_diagram(glist_source_nucleon_unpaired, glist_sink_nucleon_unpaired, glist_source_nucleon, glist_sink_nucleon,"N0"));
	...
	/*{
	  ...
	  for(int isc = 0 ; isc < 12 ; isc++){
            ...
            vectorAuxPrint.absorb(propUP_SL,isc/3,isc%3);
            vectorAuxPrint.unload();
            vectorAuxPrint.writeHDF5("propUPSL_"+spin+"_c"+col+"_t_"+std::to_string(source_local[3]));
          }
...
//          stochastic_propagator_packed.pack_propagator_from_source_to_sink(temporary, (source[3]+(j+1)*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3], max_source_sink_separations, j == 0 ? true : false);
	...
	/*if (i_sample==0){
	  stochastic_source.unload();
	  stochastic_source.writeHDF5("stochastic_vector_source.h5");
	}*/


	for (int k=0; k< tSinks.size();++k){
          int tsinkMtsource = tSinks[k];
          if(tsinkMtsource >= HGC.totalL[3])
          ...
          for (int i_source_parallel=0; i_source_parallel<parallel_sources;++i_source_parallel){
            ...
            sink_local[3] = (sourcePositions[isource][DIM_T]+tSinks[k]+i_source_parallel*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3];
	  ...
	  if (k==0 && (dotwopoint==1)){
	     ...
	     stoch_piece.apply_gamma5();
	     TIME(reductions_DD_V3_GAMMAF2U_2pt[i_sample]->V3( stoch_piece, glist_sink_meson,   propDN_SS_packed, true));
	  ...
	  PLEGMA_Vector<float> stochastic_propagator_packed;
          for (int i_source_parallel=0; i_source_parallel<parallel_sources;++i_source_parallel){
            ...
            maxinsertion_local[3] = (source[3]+(i_source_parallel+1)*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3];
	    site sink_local;
            sink_local[0] = sourcePositions[isource][0];
            sink_local[1] = sourcePositions[isource][1];
            sink_local[2] = sourcePositions[isource][2];
            sink_local[3] = (source[3]+tSinks[k]+i_source_parallel*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3];
	  ...
	  for (int i_source_parallel=0; i_source_parallel<parallel_sources;++i_source_parallel){
	    ...
	    site maxinsertion_local;
            ...
            maxinsertion_local[1] = sourcePositions[isource][1];
            maxinsertion_local[2] = sourcePositions[isource][2];
            maxinsertion_local[3] = (sourcePositions[isource][DIM_T]+(i_source_parallel+1)*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3];
            site sink_local;
            sink_local[0] = sourcePositions[isource][0];
            sink_local[1] = sourcePositions[isource][1];
            sink_local[2] = sourcePositions[isource][2];
	    ...
	    temporary.load();
	    //temporary.writeHDF5("source_for_packing_j"+std::to_string(i_source_parallel)+"_stoch"+std::to_string(i_sample));
            //temporary.load();
     ...
     ******************************************************/

      {
	   ...
	   site source_local = sourcePositions[isource];
           source_local[3]=(source[3]+i_source_parallel*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3];
	   site maxinsertion_local;
           maxinsertion_local[0] = sourcePositions[isource][0];
           maxinsertion_local[1] = sourcePositions[isource][1];
           maxinsertion_local[2] = sourcePositions[isource][2];
           ...
           //int sink_local=(sourcePositions[isource][3]+(l+1)*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3];
	   ...
	   //vectortmp1.unload();
	   //vectortmp1.writeHDF5("source_for_up_zero_mom_oet_t"+std::to_string(source_local[3]));
	   //vectortmp1.load();
	 ...
	 for (int i_source_parallel=0; i_source_parallel<parallel_sources;++i_source_parallel){
           ...
           site source_local = sourcePositions[isource];
           source_local[3]=(source[DIM_T]+i_source_parallel*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3];
	   site maxinsertion_local;

           maxinsertion_local[0] = sourcePositions[isource][0];
           maxinsertion_local[1] = sourcePositions[isource][1];
           maxinsertion_local[2] = sourcePositions[isource][2];
           maxinsertion_local[3] = (source[DIM_T]+(i_source_parallel+1)*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3];
	   ...
	   //PLEGMA_Vector<float> temporary1,temporary2;
           ...
           //temporary1.load();
           //temporary2.copy(vectortmp2);
           //PLEGMA_printf("PACKING stochastic oet dn %d\n",maxinsertion_local[3]);

           //temporary1.pack_propagator_from_source_to_sink(temporary2, maxinsertion_local[3], max_source_sink_separations, i_source_parallel == 0 ? true : false);
	   PLEGMA_Vector<float> vectorAuxF;
	   vectorAuxF.copy(vectortmp2);
      ...
      //stochastic_oet_prop_d_zero_mom_SS.unload();
      //stochastic_oet_prop_d_zero_mom_SS.writeHDF5("zero_momentum_oet_smeared_d");
      //stochastic_oet_prop_d_zero_mom_SS.load();
     ...
     ******************************************************/
     ...
     for (int k=0; k<tSinks.size();++k){
       ...
       for (int i_source_parallel=0; i_source_parallel<parallel_sources;++i_source_parallel){
         ...
         sink_local[2] = sourcePositions[isource][2];
         sink_local[3] = (source[3]+tSinks[k]+i_source_parallel*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3];
          //int global_fixSinkTime = (tsinkMtsource + source[3]+j*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3];
         propUPpacked_to_sink.pack_propagator_as_sink(propUP_SS_packed,  sink_local[3], tsinkMtsource, i_source_parallel==0 ? true : false);
         propDNpacked_to_sink.pack_propagator_as_sink(propDN_SS_packed,  sink_local[3], tsinkMtsource, i_source_parallel==0 ? true : false);
       }
	...
	PLEGMA_Vector<float> st_oet_u_zeropacked_to_sink;
        ...
        for (int i_source_parallel=0; i_source_parallel<parallel_sources;++i_source_parallel){
          ...
          sink_local[1] = sourcePositions[isource][1];
          sink_local[2] = sourcePositions[isource][2];
          sink_local[3] = (source[DIM_T]+tSinks[k]+i_source_parallel*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3];
        ...
        //TIME(reductionsV2_diluted_STOCHU_DN_UP[k]->writeHDF5("redV2_oet"));
	...
	if (k==0 && (dotwopoint==1)){
          ...
          for (int i_source_parallel=0; i_source_parallel<parallel_sources;++i_source_parallel){
            ...
            sink_local[1] = sourcePositions[isource][1];
            sink_local[2] = sourcePositions[isource][2];
            sink_local[3] = (source[DIM_T]+(i_source_parallel+1)*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3];

            //int global_fixSinkTime = (tsinkMtsource + source[3]+j*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3];
            st_oet_u_zeropacked_to_sink.pack_propagator_from_source_to_sink(stochastic_oet_prop_u_zero_mom_SS,  sink_local[3], max_source_sink_separations, i_source_parallel==0 ? true : false);
            st_oet_d_zeropacked_to_sink.pack_propagator_from_source_to_sink(stochastic_oet_prop_d_zero_mom_SS,  sink_local[3], max_source_sink_separations, i_source_parallel==0 ? true : false);
          }

	  TIME(reductionsV2_diluted_STOCHU_DN_UP_2pt.V2( st_oet_u_zeropacked_to_sink, glist_sink_nucleon, propDN_SS_packed, propUP_SS_packed, true));
          TIME(reductionsV2_diluted_STOCHD_UP_UP_2pt.V2( st_oet_d_zeropacked_to_sink, glist_sink_nucleon, propUP_SS_packed, propUP_SS_packed, true));
          TIME(reductionsV4_diluted_STOCHU_DN_UP_2pt.V4( st_oet_u_zeropacked_to_sink, glist_sink_nucleon, propDN_SS_packed, propUP_SS_packed, true));
	...
	for (int i_source_parallel=0; i_source_parallel < parallel_sources;++i_source_parallel){
	  ...
	  //pi plus at the source 
          for(int isc = 0 ; isc < 12 ; isc++){
            ...
            //Performing the smearing
            {
              ...
              vector1.absorb( vectorAuxD, (source[3]+i_source_parallel*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3]);
              PLEGMA_Gauge3D<double> smearedGauge3D;
              smearedGauge3D.absorb(smearedGauge, (source[3]+i_source_parallel*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3]);

              vector1.mulMomentumPhases(momentum_i2,1);
              TIME(vector2.gaussianSmearing(vector1, smearedGauge3D, nsmearGauss, alphaGauss));
              vectorAuxD.absorb(vector2, (source[3]+i_source_parallel*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3]);
	    }
            
	    vectorAuxD2.absorbTimeslice(vectorAuxD, (source[3]+i_source_parallel*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3], false);
	  ...
	  propTS_SS_packed.pack_propagator_from_source_to_sink(propTS_SS, (source[3]+(i_source_parallel+1)*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3], max_source_sink_separations, i_source_parallel == 0 ? true : false);
          propTS_SL_packed.pack_propagator_from_source_to_sink(propTS_SL, (source[3]+(i_source_parallel+1)*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3], max_source_sink_separations, i_source_parallel == 0 ? true : false);
	...
	for (int k=0; k<tSinks.size(); ++k){
	  ...
	  PLEGMA_Propagator<float> propTS_SS_packed_to_sink;
          ...
          for (int i_source_parallel=0; i_source_parallel< parallel_sources;++i_source_parallel){
	    int global_fixSinkTime = (tsinkMtsource + source[3]+i_source_parallel*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3];
	  ...
	  PLEGMA_Propagator<float> propUPpacked_to_sink;
          ...
          for (int i_sample=0; i_sample<n_stochastic_samples; ++i_sample){
            for (int i_source_parallel=0; i_source_parallel<parallel_sources;++i_source_parallel){
	      PLEGMA_Vector<float> temporary;
	      temporary.copy(*stochastic_propags_DN_SL[lookuptable_DN[(source[3]+tSinks[k]+i_source_parallel*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3]]*n_stochastic_samples+i_sample],HOST);
	      temporary.load();

	      stochastic_propagator_packed.pack_propagator_from_source_to_sink(temporary, (sourcePositions[isource][3]+(i_source_parallel+1)*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3], max_source_sink_separations, i_source_parallel == 0 ? true : false);
	    }
	    ...
	    PLEGMA_Vector<float> stochastic_source_packed;
            for (int i_source_parallel=0; i_source_parallel<parallel_sources;i_source_parallel++){
              stochastic_source_packed.pack_propagator_as_sink(stochastic_propagator_packed, (source[3]+tSinks[k]+i_source_parallel*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3], tSinks[k], i_source_parallel == 0 ? true : false);
            }
	...
	for (int i_source_parallel=0; i_source_parallel < parallel_sources;++i_source_parallel){
	      ...
	      PLEGMA_Gauge3D<double> smearedGauge3D;
            ...
            vectorAuxD2.absorbTimeslice(vectorAuxD, (sourcePositions[isource][3]+i_source_parallel*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3], false);
	...
	corrB13_2pt.initialize_diagram( glist_source_nucleon_unpaired, glist_sink_nucleon_unpaired, glist_source_nucleon, glist_source_meson,  glist_sink_nucleon, glist_sink_meson, "12","B13");
	  ...
	  PLEGMA_Propagator<float> propTS_SS_packed_to_sink;
          {
            for (int i_source_parallel=0; i_source_parallel<HGC.totalL[3]/max_source_sink_separations;++i_source_parallel){
              int tsinkMtsource = tSinks[k];
              if(tsinkMtsource >= HGC.totalL[3])
                PLEGMA_error("Provided tsink=%d is >= than temporal extent",tsinkMtsource);
              int global_fixSinkTime = (tsinkMtsource + source[3]+i_source_parallel*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3];
...
//	      stochastic_propagator_packed.load();
	      ...
	      temporary.load(); 
              ...
              stochastic_propagator_packed.pack_propagator_from_source_to_sink(temporary, (source[3]+(i_source_parallel+1)*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3], max_source_sink_separations, i_source_parallel == 0 ? true : false);
            }
	  ...
	  ssource=(char *)malloc(sizeof(char)*100);
	    ...
	    if (k==0 && (dotwopoint==1)){
	      ...
	      stochastic_propagator_packed.apply_gamma5();
            ...
            for (int i_source_parallel=0; i_source_parallel<HGC.totalL[3]/max_source_sink_separations;++i_source_parallel){
              PLEGMA_Vector<float> temporary;
              temporary.copy(*stochastic_propags_UP_SL[lookuptable_UP[(source[3]+tSinks[k]+i_source_parallel*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3]]*n_stochastic_samples+i_sample],HOST);
	      temporary.load();
	...
	{ //Finite momentum oet
	  ...
	  for (int i_source_parallel=0; i_source_parallel<parallel_sources;++i_source_parallel){
	    ...
	    //vectorSource_finite_mom.unload();
	    //vectorSource_finite_mom.writeHDF5("finite_mom_oet_source");
	    //vectorSource_finite_mom.load();
	    ...
	    vectortmp1.zero_device();
	      ...
	      //PLEGMA_printf("Source local %d\n",source_local[3]);
	      //vectortmp1.unload();
              //vectortmp1.writeHDF5("source_for_up_fini_mom_oet_t"+std::to_string(source_local[3]));
              //vectortmp1.load();
	    ...
	    stochastic_oet_prop_u_fini_mom_SL.pack_propagator_from_source_to_sink(vectorAuxF,(source[3]+(i_source_parallel+1)*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3], max_source_sink_separations, i_source_parallel == 0 ? true : false);

	    TIME(vectortmp2.gaussianSmearing(vectortmp1, smearedGauge, nsmearGauss, alphaGauss ));

            vectorAuxF.copy(vectortmp2);
            stochastic_oet_prop_u_fini_mom_SS.pack_propagator_from_source_to_sink(vectorAuxF,(source[3]+(i_source_parallel+1)*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3], max_source_sink_separations, i_source_parallel == 0 ? true : false);

	  }

	  //stochastic_oet_prop_u_fini_mom_SL.unload();
	  //stochastic_oet_prop_u_fini_mom_SL.writeHDF5("finite_momentum_oet_unsmeared_u");
	  //stochastic_oet_prop_u_fini_mom_SL.load();
	    ...
	    //PLEGMA_Vector<float> temporary1,temporary2;
            ...
            //temporary1.load();
            //temporary2.copy(vectortmp2);
            //temporary1.pack_propagator_from_source_to_sink(temporary2,(source[3]+(l+1)*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3], max_source_sink_separations, l == 0 ? true : false);
            //temporary1.unload();
	    ...
	    PLEGMA_Vector<float> vectorAuxF;
	    vectorAuxF.copy(vectortmp1);
	    stochastic_oet_prop_d_fini_mom_SL.pack_propagator_from_source_to_sink(vectorAuxF,(source[3]+(i_source_parallel+1)*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3],max_source_sink_separations, i_source_parallel == 0 ? true : false);

	    TIME(vectortmp2.gaussianSmearing(vectortmp1, smearedGauge, nsmearGauss, alphaGauss ));

            vectorAuxF.copy(vectortmp2);
            stochastic_oet_prop_d_fini_mom_SS.pack_propagator_from_source_to_sink(vectorAuxF,(source[3]+(i_source_parallel+1)*max_source_sink_separations+HGC.totalL[3])%HGC.totalL[3], max_source_sink_separations, i_source_parallel == 0 ? true : false);
...
/*        PLEGMA_ScattCorrelator<float> corrZ7_2pt(source, filtered_sourcemomentumList_2pt);
	...
	corrZ15_2pt.initialize_diagram( glist_source_nucleon_unpaired, glist_sink_nucleon_unpaired, glist_source_nucleon, glist_source_meson, glist_sink_nucleon, glist_sink_meson, "12", "Z5");
        corrZ16_2pt.initialize_diagram( glist_source_nucleon_unpaired, glist_sink_nucleon_unpaired, glist_source_nucleon, glist_source_meson, glist_sink_nucleon, glist_sink_meson, "12", "Z6");
/*
...
//        corrD1if12.initialize_diagram(  glist_source_nucleon_unpaired, glist_sink_nucleon_unpaired, glist_source_nucleon, glist_source_meson, glist_sink_nucleon, glist_sink_meson,"12", "MNPP02");
        ...
        if (dotwopoint==1){
	  TIME(corrZ15_2pt.Z_diagrams_without_dilution( reductionsV3_diluted_D_UP_2pt, reductionsV2_diluted_STOCHU_DN_UP_2pt, 0, 5, false ));
          TIME(corrZ16_2pt.Z_diagrams_without_dilution( reductionsV3_diluted_D_UP_2pt, reductionsV2_diluted_STOCHU_DN_UP_2pt, 0, 7, false ));
/*
