...
int main(int argc, char **argv)
{
  ...
  int NdumpStepEv=Eig_NeV;
  HGC_options->set("dump-step-ev","If accumulation is ON, Every how many stochastic vector to dump results", verbosity, NdumpStepEv);
  ...
  if(lowModesRecon){
    for(int i=0; i< eigSol->getEigVals().size(); i++){
      ...
      if((i+1)%NdumpStepEv==0){
	dumpLoops(ft_std, loopsPrefix + "/exact_part_std_NeV_"+std::to_string(i+1), confID, corr_file_format);
	dumpLoops(ft_gen, loopsPrefix + "/exact_part_gen_NeV_"+std::to_string(i+1), confID, corr_file_format);
      }
    }
    for(int i = 0 ; i < sizeFT; i++){
      ft_std[i]->zero();
