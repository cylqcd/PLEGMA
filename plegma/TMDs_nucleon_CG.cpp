...
int main(int argc, char **argv) {
  ...
  size_t B_max = 6;
  size_t L_max = HGC_totalL[WilsDir]/2;
...
//ignore this part, only used for testing purposes
  ...
  for(int isource=0;isource<numSourcePositions;isource++){
    ...
    PLEGMA_Correlator<float> corrThrpWL(corr_space, sourcePositions[isource], 0, (tSinks[0]+1));
  ...
  for(int ts=0;ts<tSinks.size();ts++) {
    ...
    PLEGMA_Propagator<float> *propF = new PLEGMA_Propagator<float>(BOTH);
      ...
      // corr.contractNucleonThrp_local(*seqPropOut1, *propF11, signProps1, gammas);
      // if(signPer < 0) for(int iv = 0 ; iv < corr.getTotalSize()*2; iv++) corr.H_elem()[iv] *= signPer;
      // corr.writeASCII( (threep_filename + ".local.CP1").c_str() );

      // corr.contractNucleonThrp_local(*seqPropOut2, *propF21, signProps2, gammas);
      // if(signPer < 0) for(int iv = 0 ; iv < corr.getTotalSize()*2; iv++) corr.H_elem()[iv] *= signPer;
      // corr.writeASCII( (threep_filename + ".local.CP2").c_str() );
    ...
    for(int b = 0; b <= B_max; b++) {
      ...
      for(int z = 0; z <= Z_max; z++) {
        if(z != 0) {
          propExchange = propIn; propIn = propF; propF = propExchange;
          propF->shift(*propIn, 4 * zdir + WilsDir);
        }
        if(zrev) zval = Z_max - z;
        else zval = z;
    
        corrThrpWL.contractNucleonThrp_staple(*seqPropOut1, *propF, *WL, signProps1, gammas, l, b, z);
        ...
        corrThrpWL.contractNucleonThrp_staple(*seqPropOut2, *propF, *WL, signProps2, gammas, l, b, z);
        if(signPer < 0) for(int iv = 0 ; iv < corrThrpWL.getTotalSize()*2; iv++) corrThrpWL.H_elem()[iv] *= signPer;
