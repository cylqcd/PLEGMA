...
//     options.set("nsmear-gauss-c", "Number of Gaussian smearing step for the charm quark propagator", verbosity, nsmearGauss_c);
 ...
 //   PLEGMA_Gauge<float> contractGauge(BOTH);
  ...
  //    applyBoundaryConditions(contractGauge,true);
    ...
    PLEGMA_Propagator<float> propST[storedProps];
...
//                 prop.applyBoundaries_device(source[DIM_T]);
        ...
        TIME(computeStochasticPropagator(propST[indNew], kappa_s, STRANGE, nsmearGauss_s));
...
//     PLEGMA_printf("corr.getSiteSize(): %d\n", corr.getSiteSize());
            ...
            if (isource == (storedProps-1)){
                ...
                for (int randInd1=0; randInd1< storedProps; randInd1++){
                    for (int randInd2=0; randInd2< storedProps; randInd2++){ 
                        ...
                        std::string group_name = "tetraquarks/r"  + std::to_string(totalInd1) +"r"  + std::to_string(totalInd2);
                        
                        if (randInd1 != randInd2){
                            
                            TIME(corr.contractTetraquarksStochastic(propLT[randInd1], propLT[randInd2],propST[randInd1], propST[randInd2], propBT[randInd1], propBT[randInd2])); 
                        ...
                        for (int i=0; i<gammas.size(); i++){
                            ...
                            for (int s1=0; s1< N_SPINS; s1++){  // this sum is required as the GPU memory is not large enough to cover all 4*4*3*3 elements in double precision
                                TIME(corr.contractTetraquarkScatteringOpenIndexStochastic(propBT[randInd1], propLT[randInd2], gamma, totalInd1, totalInd2, s1, "BU"));
                                THREAD(corr.writeFile(openContr_filename, corr_file_format));
                                
                                TIME(corr.contractTetraquarkScatteringOpenIndexStochastic(propBT[randInd1], propST[randInd2], gamma, totalInd1, totalInd2, s1, "BS"));
                                THREAD(corr.writeFile(openContr_filename, corr_file_format));
                            }
                        }
            ...
            if (isource >= (storedProps)){
                ...
                for (int randInd=0; randInd< storedProps; randInd++){
                        ...
                        if (randInd != indNew){
                            std::string group_name = "tetraquarks/r"  + std::to_string(totalInd) +"r"  + std::to_string(totalIndNew);
                            TIME(corr.contractTetraquarksStochastic(propLT[randInd], propLT[indNew],propST[randInd], propST[indNew], propBT[randInd], propBT[indNew])); 
                            asprintf(&group, group_name.c_str());
                            corr.setGroups(group);
                            free(group);
                            THREAD(corr.writeFile(twop_filename, corr_file_format));  
                            
                            group_name = "tetraquarks/r"  + std::to_string(totalIndNew) +"r"  + std::to_string(totalInd);
                            TIME(corr.contractTetraquarksStochastic(propLT[indNew], propLT[randInd],propST[indNew], propST[randInd], propBT[indNew], propBT[randInd])); 
                            asprintf(&group, group_name.c_str());
                            corr.setGroups(group);
                            free(group);
                            THREAD(corr.writeFile(twop_filename, corr_file_format));
                            ...
                            for (int i=0; i<gammas.size(); i++){
                                ...
                                for (int s1=0; s1< N_SPINS; s1++){
                                    TIME(corr.contractTetraquarkScatteringOpenIndexStochastic(propBT[randInd], propLT[indNew], gamma, totalInd, totalIndNew, s1, "BU"));
                                    THREAD(corr.writeFile(openContr_filename, corr_file_format));
                                
                                    TIME(corr.contractTetraquarkScatteringOpenIndexStochastic(propBT[indNew], propLT[randInd], gamma, totalIndNew, totalInd ,s1, "BU"));
                                    THREAD(corr.writeFile(openContr_filename, corr_file_format));
                                    
                                    
                                    TIME(corr.contractTetraquarkScatteringOpenIndexStochastic(propBT[randInd], propST[indNew], gamma, totalInd, totalIndNew, s1, "BS"));
                                    THREAD(corr.writeFile(openContr_filename, corr_file_format));
                                
                                    TIME(corr.contractTetraquarkScatteringOpenIndexStochastic(propBT[indNew], propST[randInd], gamma, totalIndNew, totalInd ,s1, "BS"));
                                    THREAD(corr.writeFile(openContr_filename, corr_file_format));
                                }
                            }
                        }
                    ...
                    std::vector<GAMMAS> gammas = {G1,G2,G3,G5};
                        ...
                        for (int i=0; i<gammas.size(); i++){
                            std::vector<GAMMAS> gamma = {gammas[i]};
                            for (int s1=0; s1< N_SPINS; s1++){
                                TIME(corr.contractTetraquarkScatteringOpenIndexStochastic(propBT[indNew], propLT[indNew], gamma, totalIndNew, totalIndNew, s1, "BU"));
                                THREAD(corr.writeFile(openContr_filename, corr_file_format));
                                
                                TIME(corr.contractTetraquarkScatteringOpenIndexStochastic(propBT[indNew], propST[indNew], gamma, totalIndNew, totalIndNew, s1, "BS"));
                                THREAD(corr.writeFile(openContr_filename, corr_file_format));
                            }
