...
int main(int argc, char **argv) {
  ...
  size_t B_max = 0; //6;
  size_t L_max = 0; //Lvals[Lvals.size()-1];
  size_t Z_max = 0; //HGC_totalL[WilsDir]/2;
...
//ignore this part, only used for testing purposes
  ...
  for(int ts=0;ts<tSinks.size();ts++) {
    ...
    suff22 = "_CP2_stout_"+std::to_string(nStout)+"_Bdir_"+std::to_string(Bdir2)+"_Plus_";
            ...
            TIME(corrThrpWL.contractNucleonThrp_staple(*seqPropOut1, *propF, *WL_1, signProps1, gammas, 0, 0, 0));
            ...
            TIME(corrThrpWL.contractNucleonThrp_staple(*seqPropOut2, *propF, *WL_1, signProps2, gammas, 0, 0, 0));
            if(signPer < 0) for(int iv = 0 ; iv < corrThrpWL.getTotalSize()*2; iv++) corrThrpWL.H_elem()[iv] *= signPer;
      ...
      if(std::count(Lvals.begin(), Lvals.end(), l)) {//calculate only for certain l's
        ...
        for(int b = 0; b <= B_max; b++) {//loop over values of B, building "2"
          ...
          for(int z = 0; z <= Z_max; z++) {//loop over values of Z, building "4"
            ...
            TIME(corrThrpWL.contractNucleonThrp_staple(*seqPropOut1, *propF, *WL_4_1, signProps1, gammas, l, b, z));
            if(signPer < 0) for(int iv = 0 ; iv < corrThrpWL.getTotalSize()*2; iv++) corrThrpWL.H_elem()[iv] *= signPer;
            corrThrpWL.writeHDF5( (threep_filename + suff11 + "_ts_" + std::to_string(tSinks[ts]) + ".h5").c_str());
            ...
            TIME(corrThrpWL.contractNucleonThrp_staple(*seqPropOut1, *propF, *WL_4_2, signProps1, gammas, l, b, z));
            if(signPer < 0) for(int iv = 0 ; iv < corrThrpWL.getTotalSize()*2; iv++) corrThrpWL.H_elem()[iv] *= signPer;
            ...
            TIME(corrThrpWL.contractNucleonThrp_staple(*seqPropOut2, *propF, *WL_4_1, signProps2, gammas, l, b, z));
            if(signPer < 0) for(int iv = 0 ; iv < corrThrpWL.getTotalSize()*2; iv++) corrThrpWL.H_elem()[iv] *= signPer;
            corrThrpWL.writeHDF5( (threep_filename + suff21 + "_ts_" + std::to_string(tSinks[ts]) + ".h5").c_str());
            ...
            TIME(corrThrpWL.contractNucleonThrp_staple(*seqPropOut2, *propF, *WL_4_2, signProps2, gammas, l, b, z));
            if(signPer < 0) for(int iv = 0 ; iv < corrThrpWL.getTotalSize()*2; iv++) corrThrpWL.H_elem()[iv] *= signPer;
