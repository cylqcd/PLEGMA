...
namespace plegma {
  ...
  protected:
    ...
    Float *corr(  std::initializer_list<int> idx ){
      std::vector<int> tmp(offsets.size());      
      std::transform(idx.begin(),idx.end(),offsets.begin(),tmp.begin(),std::multiplies<int>());
      
      return H_elem() + std::accumulate(tmp.begin(), tmp.end(), 0);
    }
