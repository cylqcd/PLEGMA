...
__global__ void contract_mesons_SIB_device( propTex<FloatA> texProp1,
...
#pragma unroll
	for(int s = 0 ; s < 5; s++){
	  ...
	  accum[s] = accum[s] + value * prop1[alpha][beta][b][s1] * conj(prop2[delta][gamma][b][s2]);
	}
    }
  }
  if(runFT) {
...
template<typename FloatA, typename FloatB, typename FloatC>
void contract_mesons_SIB_host( ProfileStruct &ps,
  ...
  for(int it=0; it < t_size; it+=time_step) {
	...
	contract_mesons_SIB_device<FloatA, FloatB, FloatC>
...
template<typename FloatA, typename FloatB, typename FloatC>
static void contract_mesons_SIB(PLEGMA_Propagator<FloatA>& prop1, PLEGMA_Propagator<FloatB>& prop2,
				PLEGMA_Correlator<FloatC>& corr){
  bool runFT = (corr.getCorrSpace()==MOMENTUM_SPACE);
  int site_size = 5*N_MESONS;
  ...
  tuneAndRun( ps, "contract_mesons_SIB", contract_mesons_SIB_host<FloatA,FloatB,FloatC>,
