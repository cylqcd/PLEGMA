...
static void contract_exact_exact(Float* ptr, int nvecs, size_t vec_size, bool dev_ptr,
  ...
  for(int i=0; i<nvecs; i++) {
	...
	qudaMemcpyAsync(vec2[k]->D_elem(), ptr+(i+k+1)*vec_size, vec_bytes, qudaMemcpyHostToDevice, stream[k]);
      }
    }
    ...
    if(dev_ptr) {
      ...
      qudaMemcpy(vec1.D_elem(), ptr+i*vec_size, vec_bytes, qudaMemcpyHostToDevice);
      //checkCudaError();
    }
	...
	if(j+ils<nvecs) {
	  if(dev_ptr) {
	    ...
	    qudaMemcpyAsync(vec2[k]->D_elem(), ptr+(j+ils)*vec_size, vec_bytes, qudaMemcpyHostToDevice, stream[k]);
	  }
	}
      }
      veci++;
