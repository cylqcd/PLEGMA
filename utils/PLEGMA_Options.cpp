...
void plegmaOptions(Options &opt){
  ...
  opt.set("load-gauge", "Path to the gauge field, default (empty string)", verbosity, latfile);

  // smearing
  opt.set("nsmear-APE", "Number of APE smearing step", verbosity, nsmearAPE);
  opt.set("alpha-APE", "Coefficient for the APE smearing", verbosity, alphaAPE);
  opt.set("nsmear-gauss", "Number of Gaussian smearing step", verbosity, nsmearGauss);
  opt.set("alpha-gauss", "Coefficient for the Gaussian smearing", verbosity, alphaGauss);
  opt.set("nsmear-stout", "Number of stout smearing step", verbosity, nsmearStout);
  opt.set("alpha-stout", "Coefficient for the stout smearing", verbosity, alphaStout);

  // sources
  opt.set("nsrc", "Number of source positions", verbosity, numSourcePositions);
  isFound = opt.set("src-filename", "Filename of source positions", verbosity, pathListSourcePositions);
  if(isFound) readSourceList();
  
  // Correlators
  opt.set("maxQsq", "Maximum Qsq for the Fourier Transform", verbosity, maxQsq);
  opt.set("twop-filename", "File name for two-point functions, extension will be added", verbosity, twop_filename);
  opt.set("threep-filename", "File name for three-point functions, extension will be added", verbosity, threep_filename);
  ...
  opt.set("n-tsink", "Number of T sinks", verbosity, numTSink);
  isFound = opt.set("tsink-filename", "Filename of list of T sink", verbosity, pathListTSink);
  if(isFound) readTSinkList();

  opt.set("n-projs", "Number of projectors", verbosity, numProj);
...
void qudaOptions(Options &opt){
  ...
  isFound=opt.set("Q-mg-block-size", "Set the geometric block size for the each multigrid level's transfer operator (default 4 4 4 4)", verbosity, tpl_int_site);
