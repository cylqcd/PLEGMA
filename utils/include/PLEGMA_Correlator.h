...
namespace plegma {
  ...
  public:
    ...
    PLEGMA_Correlator(PLEGMA_Correlator<Float> corr, int size):
      source(corr.source), totalT(corr.totalT), corr_space(corr.corr_space), corr_pos_space(corr.corr_pos_space), corr_mom_space(corr.corr_mom_space) {
      shape = corr.getShape();
      shape.insert(shape.begin(), size);
      datasets =  corr.getDatasets();
      groups =  corr.getGroups();
    ...
    void insert(PLEGMA_Correlator<Float> corr, int index);
